...
**Impact**: A frame that misses the interval leaves the previous frame on the display. A frozen instrument looks like a working one, so the driver gets no indication that the reading is old.
