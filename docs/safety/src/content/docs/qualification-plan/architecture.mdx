...
The element reference in the [Reference](/reference/) is generated from the `\sc` markers in the compiler's builtins, so it lists exactly what the subset contains.

:::caution[TODO]
`slint_build` doesn't support Slint SC yet, so there's no build-script integration; the compiler binary is invoked directly.
Decide whether `slint_build` gains a safety-critical mode, or the manual documents the direct invocation.
:::
...
Three properties of the design hold by construction, so nothing has to be done to maintain them and no constraint on the integrator depends on them.
