...
6. Measures the coverage of the case's `.slint` code and compares it with the caret lines the case states it in, if it states it.
   Every case that states it tests that reporting.
   The cases under `coverage/` test nothing else, so they have to state theirs; see [Coverage Tool Verification](/qualification-plan/slint-coverage/).
...
From the code up, every line, function, code region, and branch outcome of the runtime is executed by the suites.
