...
[Coverage of Slint Code](/user-manual/slint-coverage/) in the user manual says what it measures and how.
...
Every case with caret lines tests the reporting, since the driver compares what it states with the measurement.
The cases under `coverage/` exist to test the reporting, so the driver also fails one of them that states no coverage.
Each case that states its coverage pins one part of the reporting:
...
| `coverage/multiline` | a decision written over several lines, whose outcomes are points of the line of its operator |
| `coverage/state` | a state: its name, the `when` of its condition, the decisions it's written with, and the value it gives each property |
