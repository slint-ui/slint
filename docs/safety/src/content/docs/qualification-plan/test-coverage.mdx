...
The compiler writes, next to the generated code, a map from ranges of the code to the coverage points of the
`.slint` source: every element, binding, callback handler and call, and both outcomes of every `?:`, `&&`, and
`||` decision.
The generated code carries nothing, so the code measured is the code that ships, not an instrumented build of it.
The generated code is measured by LLVM like any other, and the `slint-sc-coverage` tool maps the execution count
of the code at each range back to the `.slint` files, in the lcov format, alongside the Rust coverage.
