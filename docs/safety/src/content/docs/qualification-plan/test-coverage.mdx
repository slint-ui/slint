...
The runtime's own test cases are written in Slint and measured this way on every run.
The [Slint Code Coverage](/qualification-report/slint-coverage/) chapter of the qualification report holds the result.
Complete coverage isn't asked of them: the cases under `coverage/` exist to pin what an unreached point looks like.
