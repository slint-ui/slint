...
| `slint.compiler.avd.spec-tests` | Every paragraph of the language specification is traced to at least one test, and the coverage of the test run shows what that test reached. See the [Traceability Matrix](/qualification-report/traceability-matrix/) and [Slint Code Coverage](/qualification-report/slint-coverage/). |
| `slint.runtime.avd.spec-tests` | Every paragraph of the rendering model and the generated code contract is traced to at least one test, and the runtime is held at complete structural coverage. See [Test Coverage](/qualification-report/test-coverage/) for the runtime and [Slint Code Coverage](/qualification-report/slint-coverage/) for the `.slint` code of the tests. |
