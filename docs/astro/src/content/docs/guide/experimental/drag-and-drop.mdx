...
fn main() {
    ...
    api.on_can_drop(|data| {
        // The set of plaintext and image MIME types that are supported by the
        // helper methods is available as an associated constant.
        data.mime_types()
            .contains(|ty| slint::DataTransfer::PLAINTEXT_MIME_TYPES.contains(&ty)))
    });
    // ...
}
```
