...
description: Experimental match expression for conditional element rendering.
---

A `match` expression conditionally renders elements from a set of cases based on the value of an expression.
...
The match expression feature is in its early stages. The following restrictions apply:
...
- `match` cannot appear in global components or interfaces. Only regular components support match statements.

- No exhaustiveness checking. The match expressions do not verify that all possible values are covered. A missing case simply renders nothing.
