...
Each `<case-value>` is a literal that is compared against the `<match-subject>`. If a wildcard case `*` is present, it renders when none of the explicit cases match.
