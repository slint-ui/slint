...
A literal subject (for example `match 1 { … }`) warns, because the same case always applies.
...
- Float comparisons produce an error. Comparing floating-point values for exact equality is not reliable, so the compiler emits an error for each float case value.
