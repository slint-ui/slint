...
The `@tr` macro supports `{}` placeholders, plural forms, and translation contexts;
the [Macros](../../../reference/language/macros/#tr) chapter of the language reference describes the full syntax.
Without an explicit context, the context defaults to the name of the surrounding component.
