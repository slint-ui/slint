...
The current used decimal separator can be read with [decimal-separator](/src/content/docs/reference/global-namespaces/platform#decimal-separator). As default the dot `.` is used as decimal separator.
To set a custom decimal separator for every language add the following to your `.po` files

```
msgctxt "SlintDecimalSeparator"
msgid "."  # keep the default here
msgstr "," # <---------- Change to the desired decimal separator
```
