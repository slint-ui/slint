...
`can-drop` returns a <Link type="DragAction" />: `DragAction.copy`, `DragAction.move`, `DragAction.link` to accept the drag with that action, or `DragAction.none` to reject it.
...
The target picks the final action in `can-drop`, the runtime clamps it against the source's allowed set, and when the drop completes the source's `drag-finished(action)` fires so a "move" source can remove the original data.
...
DropArea {
    ...
    dropped(event) => Api.accept-drop(event.data);
}
```
