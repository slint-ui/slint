...
The Skia and Software renderers support an opt-in `occlusion-culling` feature that skips drawing
items fully hidden behind opaque content, at the cost of an extra tree walk and per-item
bookkeeping on every frame. It isn't available with the FemtoVG or Qt renderers.

<Tabs syncKey="dev-language">
<TabItem label="Rust" icon="seti:rust">
Enable the <LangRefLink lang="rust-slint" relpath="docs/cargo_features/">`occlusion-culling`</LangRefLink> Cargo feature on the `slint` crate.
</TabItem>
<TabItem label="C++" icon="seti:cpp">
When building Slint from source, enable the <LangRefLink lang="cpp" relpath="cmake/#features">`SLINT_FEATURE_OCCLUSION_CULLING`</LangRefLink> cmake option.
</TabItem>
</Tabs>
...
- Public <LangRefLink lang="rust-slint" relpath="platform/software_renderer/">Rust</LangRefLink> and <LangRefLink lang="cpp" relpath="api/slint/platform/softwarerenderer/">C++</LangRefLink> API.
