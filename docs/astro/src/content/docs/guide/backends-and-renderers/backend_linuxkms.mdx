...
CMake), which links it via pkg-config. Set the `DRM_LEASE_NAME` environment variable to the lease
name managed by the daemon (for example `card0-HDMI-A-1`); the backend then requests that lease and
renders on it. The variable carries no `SLINT_` prefix because the name is a drm-lease-manager
concept: a launcher can set it once for every application it starts. `SLINT_DRM_LEASE_FD` takes
