...
While the event loop runs, Slint switches off the keyboard of the active virtual console,
so that keystrokes don't also reach the shell on that console.
If the application is killed and the console keyboard stays off, restore it with `kbd_mode -u`.
:::
