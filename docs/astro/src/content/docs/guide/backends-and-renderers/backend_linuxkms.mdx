...
to use Skia without a working graphics acceleration driver.
Select `SLINT_BACKEND=linuxkms-skia-software` to request this renderer explicitly,
including when other Skia renderers are enabled.
This feature does not add software rendering support to other backends.
