...
### Using `slint-lsp` with pre-commit

You can run Slint formatting automatically on `git commit` with [pre-commit](https://pre-commit.com/).

#### Option 1: Use the shared hook repo

Use [KDABLabs/slint-pre-commit-hooks](https://github.com/KDABLabs/slint-pre-commit-hooks) to get a version-pinned, isolated hook setup:
...
#### Option 2: Local / system hook

If `slint-lsp` is already installed in your `$PATH` (for example with `cargo install slint-lsp`), you can use a local hook:
...
This is simpler, but you manage the `slint-lsp` version yourself.
