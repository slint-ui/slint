...
The easiest way to install `slint-lsp` is with [cargo binstall](https://github.com/cargo-bins/cargo-binstall), which downloads a pre-built binary instead of compiling from source:

```sh
cargo binstall slint-lsp
```

If you prefer to compile from source, use `cargo install`:
...
Alternatively, you can manually download a pre-built binary:
