...
This following section gives you an insight into the thinking behind the language and the core concepts it is made up from.
...
```
This simple example shows the core of how Slint works. You name the element or component you want e.g. `Text`.
This is followed by open and closed braces `Text {}`. Then within the braces you customize the element or component
to your need with properties e.g. `font-size: 24px`.

To nest elements inside each other they simply are placed within the parents braces. For example the following
...
In this simple example a property called `counter` is declared. Then the `Rectangle` has a 
...
The Text elements `text` property depends on the `counter` property. When it changes the UI is automatically
updated. There is no need to opt in. In Slint every expression is automatically re-evaluated. However this
is done in a performant way that only updates expressions where the dependencies have changed.

Slint makes it trivial to create your own components by composing together the built in elements or other compoents. 
...
export component MyView {
  Dialog {
    title: "Can UI Developement Be Easy?";
    Button {
      text: "Yes";
    }
  }
...
The end result is with practice you can reduce any level of complexity to easy to manage and maintainable UI
components.
