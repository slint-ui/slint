...
The concepts section of the guide are to give you an overview of the language from a high level. If you want to
dive straight in then the details then the <Link type="slintFile" label="coding section"/> and language reference
sections are the best places to start.

This section is to give you an insight into the thinking behind the language and the core concepts it is made up from.



## Why Slint?

The Slint language helps you to describe the User Interface of your application in a declarative way.

The reasons for this are that first a User Interface is quite different from abstract code. It's made up 
...
On line 2 we declare a Button. On line 3 its `text` property is set to `"Click me!"` and on line 4 
...
Similarly that when the clicked callback activates it will run the `debug` function. There is no need to think
