...
A `width` or `height` given as a percentage is relative to the parent element:
`width: 50%;` is short for `width: parent.width * 50%;`.
See [Geometry](../../../../reference/language/geometry/) in the language reference.
...
An element's own stretch factor is the smallest among its own and its children's.
Set `horizontal-stretch` or `vertical-stretch` on the element to override.
...
The `@children` placeholder controls where a component places the child elements of its use site;
see [Container Components](../../../../reference/language/container-components/) in the language reference.
