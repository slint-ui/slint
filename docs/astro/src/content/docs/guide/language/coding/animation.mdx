...
The bounce of a spring. Values between 1 and 0 are bouncy (underdamped), 0 is smooth (critically damped), and values from 0 to -1 are slow (overdamped). Springs with `duration` and `bounce` aren't guaranteed to be finished after `duration` is over, but will be close to done.
</SlintProperty>
