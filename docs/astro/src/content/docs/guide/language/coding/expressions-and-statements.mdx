...
Predicate expressions use the syntax `(name) => expression`.
The `name` is available only in the expression after `=>`.
Predicates are currently used as arguments to array and model operations such as
`array.any((value) => value > 0)` and `array.all((value) => value > 0)`,
where the expression must evaluate to `bool`.
