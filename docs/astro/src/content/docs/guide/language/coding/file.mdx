...
:::note[Note]
Names have to be valid [identifiers](../../../../reference/language/lexical-structure/).
:::
...
Components declared in a `.slint` file can be used as elements in other `.slint` files,
...
Elements, globals and structs can be exported and imported as well.
For all the forms, including renaming and re-exports, see the
[Imports](../../../../reference/language/imports/) and [Exports](../../../../reference/language/exports/)
chapters of the language reference.
...
A library provides images and fonts as well as components.
Reference them with the same prefix, as in `@image-url("@mylibrary/logo.png")`
or `import "@mylibrary/MyFont.ttf";`.

The path to each library, as file or directory, must be defined separately at compilation time.
To define them for your build and your editor at once, set `library-paths` in a [project file](../../../tooling/project-file/).
