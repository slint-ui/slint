...
-   **`array.length`**: Use the builtin `.length` property to query the length of an array or model.
-   **`array[index]`**: The index operator retrieves individual elements of an array.
-   **`array.push(value)`**: Adds an element to an array or model.
-   **`array.remove(index)`**: Removes an element from an array or model. The accepted index range is `0 <= index < array.length`.
-   **`array.insert(index, value)`**: Inserts an element at a specified index in an array or model. The accepted index range is `0 <= index <= array.length`.
...
export component Example {
    in-out property<[int]> list-of-int: [1,2,3];
    init => {
        // Both operations do nothing and the list-of-int property value remains [1, 2, 3].
