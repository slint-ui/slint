...
See <Link type="StyleWidgets" label="Selecting a Widget Style"/> for details how to select the style and how to use the `Palette` and `StyleMetrics` properties. If no style is selected, `fluent` is the default on all platforms.
