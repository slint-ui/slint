...
`current-index` and `current-value` each resolve the other to match on every write, so of
two writes made right after each other, whichever came last determines the final state —
there's no inherent priority between the two properties.
