...
:::note
Writing to `current-value` — from host code (Rust, C++, JavaScript, Python) or imperatively
from `.slint` code — looks up the written value in `model` and moves `current-index` to the
first matching row. If the value isn't found in `model`, the selection is cleared
(`current-index` becomes `-1` and `current-value` becomes `""`), the same as setting
`current-index` out of range. Prefer writing `current-index` directly when the row's
...
one is currently selected — is a no-op and does not move the selection. A `current-value`
set declaratively at instantiation (as in the example above) replaces the default binding
without going through this lookup, so `current-index` is left at its default rather than
being resolved from `model` — prefer `current-index` for declarative initial selection.
:::
</SlintProperty>
