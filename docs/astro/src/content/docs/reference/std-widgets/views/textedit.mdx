...
### focus()
Call this function to focus the TextEdit and make it receive future keyboard events.

### clear-focus()
Call this function to remove keyboard focus from this `TextEdit` if it currently has the focus. See also <Link type="FocusHandling" label="focus handling" />.
