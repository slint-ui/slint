...
The `FocusScope` can react to <Link type="Keys" label="keyboard shortcuts"/> using the <Link type="Shortcut" label="Shortcut element"/>, and exposes callbacks to handle key events manually.
...
See <Link type="Keys" label="Keyboard Shortcuts"/> for details.
