...
### focus-changed-event(FocusEventReason)
Invoked when the focus on the `FocusScope` has changed. The argument is a a <Link type="FocusEventReason" /> enum containing the reason for focus change.

### focus-gained(FocusEventReason)
Invoked when the `FocusScope` gains focus. The argument is a a <Link type="FocusEventReason" /> enum containing the reason for focus gain.

### focus-lost(FocusEventReason)
Invoked when the `FocusScope` loses focus. The argument is a a <Link type="FocusEventReason" /> enum containing the reason for focus loss.