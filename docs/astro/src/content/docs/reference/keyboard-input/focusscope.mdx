...
The `FocusScope` can react to <Link type="KeyboardShortcuts" label="keyboard shortcuts"/> using the <Link type="Shortcut" label="Shortcut element"/>, and exposes callbacks to handle key events manually.
...
Place `Shortcut` elements inside a `FocusScope` to declare keyboard shortcuts.

See <Link type="KeyboardShortcuts" label="Keyboard Shortcuts"/> for details.

### Properties of `Shortcut`

#### keys

<SlintProperty propName="keys" typeName="keyboard-shortcut">
The <Link type="keyboard-shortcut" label="keyboard-shortcut" /> to match against incoming key events.
</SlintProperty>

#### enabled
<SlintProperty propName="enabled" typeName="bool">
Whether this Shortcut is currently enabled. Disabled Shortcut elements do not consume key events and never invoke their `activated()` callback.
</SlintProperty>
...
Invoked when the parent `FocusScope` receives a key event that matches the `keys` of this `Shortcut`.
