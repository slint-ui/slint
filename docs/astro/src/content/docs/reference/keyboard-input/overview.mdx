...
To handle keyboard shortcuts, Slint uses a built-in <Link type="keys" label="keys" /> type, which is constructed from the `@keys` macro.
...
The Shortcut's `keys` property takes a `keys` and invokes the `activated` callback when the shortcut is detected.
...
Keyboard shortcuts in Slint are based on **logical keys** — the character a keypress produces on the current keyboard layout — not the physical position of a key.
...
If you need to define a keyboard shortcut that involves a key outside of the [`Key` namespace](#key-namespace), continue reading the next section which covers advanced keyboard shortcuts.
...
For example, to support keyboard shortcuts that produce a Euro-Sign (€), add both `Shift?` and `Alt?`: `@keys(Control + Shift? + Alt? + "€")`.
