...
---
{/* cSpell: ignore Backtab WASD */}

import Link from '@slint/common-files/src/components/Link.astro';
import SpecialKeys from "/src/content/docs/generated/reference/enums/_keys.md"
...
The `keys` type represents a key, combined with modifiers and is constructed with either the `@keys` macro for logical keys or the `@physical-keys` macro for [physical key positions](#physical-key-bindings).

Key bindings created with `@keys` are based on **logical keys** — the character a keypress produces on the current keyboard layout — not the physical position of a key.
...
See the [section on physical keys](#physical-key-bindings) if you need to match a key binding based on the keys position instead.
