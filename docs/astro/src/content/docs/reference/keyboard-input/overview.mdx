...
The Shortcuts `keys` property takes a `keyboard-shortcut` and provides an `activated` callback when the shortcut is detected.
...
- `Meta` (⌃ control on MacOS - Windows key on PC)
...
You can also use the `matches(..)` function on a `keyboard-shortcut` to check whether a given [`KeyEvent`](#keyevent) matches the shortcut.
