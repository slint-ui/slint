...
- [`@image-url`](../../property-types/images/#image), [`@markdown`](../../property-types/strings/#styled-text), and [`@keys`](../../property-types/builtin-types/#keys) build an `image`, `styled-text`, or `keys` value.
