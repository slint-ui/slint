...
-   Struct types convert to another struct type when the fields they have in common convert.
    A missing field takes its default value, and an extra field is discarded with a warning.
