...
-   Struct types convert to another struct type when the properties they have in common convert.
    A missing property takes its default value, and an extra property is discarded with a warning.
    Having both at once is an error, because it is likely a typo.
...
export component Example {
    ...
    // OK, with a warning: the extra c is discarded
    ...
    // OK: no field listed, so each one takes its default value
    property<{a: string, b: int}> prop5: {};
