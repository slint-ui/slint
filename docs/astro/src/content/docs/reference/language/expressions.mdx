...
<OnlyInSC>
The compiler reads and decodes the image file when it generates code, so the
file must exist and be readable then, and isn't needed at run
time. \{#sls.expr.image.compile-time}

The file shall hold a raster image in a format the compiler can decode, such
as PNG or JPEG.
