...
Slint follows the operating system's reduced-motion setting.
While it is on, every `animate` block behaves as if `enabled` were `false`: a change sets the property to its target value at once.
A change of the setting applies to animations that start afterwards; one already running finishes at its own pace.
A `Flickable` still pans while the pointer is down, since that is the user's own movement, but a mouse wheel step lands at once and the content stops as soon as the pointer is released.
Effects driven by `animation-tick()` are not animations of a property change and are left as they are.

The setting is read at startup and followed while the application runs.
It is currently honored on Windows ("Animation effects" under Settings > Accessibility > Visual effects) with the winit backend.

A build for a device without such a setting, such as a microcontroller, can fix the value at compile time (`with_reduced_motion` in `slint-build`, `--reduced-motion` for `slint-compiler`, the `SLINT_REDUCED_MOTION` CMake target property).
Animations then do not read the setting at run time.
