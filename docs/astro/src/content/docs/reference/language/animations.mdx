...
The `spring` easing curve can be defined as `spring`, `spring()`, and `spring(bounce)` where `spring`, `spring()`, and `spring(0)` are equivalent. `bounce` must be between `-1` and `1`; any other value is a compile error.
...
A spring cannot oscillate forever. It is clamped to at most (`iteration-count` + 9) x duration.
It will oscillate at `bounce` for `iteration-count` and then `bounce` will be decreased such that it will settle within 9x duration.
If `iteration-count` is not set (defaults to 1), the spring will oscillate for at most 10x duration.
If infinite springs are required, set `bounce` to 1 and `iteration-count` to -1. See <Link type="iterations-and-directions" label="Iterations and Directions" /> below for why.
...
is not `alternate` or `alternate-reverse`, an `iteration-count` of -1 will be an infinite spring. If `bounce` is set to values
