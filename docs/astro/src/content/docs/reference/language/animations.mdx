...
### Details
The spring's animation relies on the [natural frequency](https://en.wikipedia.org/wiki/Natural_frequency)(ω₀) and the [damping ratio](https://en.wikipedia.org/wiki/Damping#Damping_ratio)(ζ), derived from `bounce` and `duration`, to feed the [mass-spring-damper model](https://en.wikipedia.org/wiki/Mass-spring-damper_model).
