...
An `in` property is an input of the component. \{#sls.prop.decl.in}

On the root element of a component the application instantiates directly, the application sets
its value through the generated API. \{#sls.prop.decl.in.root}

On any other element, the enclosing component sets its value with a binding. \{#sls.prop.decl.in.child}
...
<SC>
<OnlyInSC>
The declared type can be any valid property type. \{#sls.prop.decl.types}
</OnlyInSC>
</SC>
...
A declaration may be preceded by a [`@deprecated` annotation](../deprecation/),
which marks the property as deprecated and points users at its replacement.
