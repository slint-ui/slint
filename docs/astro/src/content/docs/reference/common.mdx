...
The stacking order of the element among its siblings.
A sibling with a higher `z` is drawn on top, and is the first to receive input.
The default is 0, and siblings that share the same `z` keep their declaration order.
</SlintProperty>
...
<SlintProperty typeName="bool" propName="accessible-item-selectable" >
