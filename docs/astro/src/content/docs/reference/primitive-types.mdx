...
To specify an easing curve, use the values from the `Easing` namespace. Additionally, in expressions of type `easing`, the following names are in scope (see [`easings.net`](https://easings.net/) for a visual reference):
