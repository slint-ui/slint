...
### MouseCursor
<SlintProperty propName="mouse-cursor" typeName="MouseCursor" defaultValue='BuiltInMouseCursor.default'>
The `MouseCursor` type is used in the <Link type="TouchArea" label="TouchArea element"/>.

Built-in cursor shapes can be specified with or without the `MouseCursor` namespace. See <Link type="BuiltInMouseCursor" label="BuiltInMouseCursor" /> for a list of built-in shapes.
