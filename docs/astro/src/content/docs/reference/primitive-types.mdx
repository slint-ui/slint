...
### keyboard-shortcut
<SlintProperty propName="keyboard-shortcut" typeName="keyboard-shortcut" defaultValue='empty shortcut'>
A keyboard-shortcut represents a key combined with a list of modifiers.
This is the primitive type used to detect if a KeyEvent should trigger a given shortcut.

Keyboard shortcuts in Slint are based on **logical keys** — the character a key produces on the current keyboard layout — not the physical position of a key on the keyboard.
