...
### @radial-gradient(circle [at x y] [size radius], color percentage, color percentage, ...)
...
The optional `size` parameter sets the circle radius in the element's local coordinate space.
The `radius` value is a length expression.
If omitted, the radius is half of the element's bounding box diagonal.
You can use `size` without `at` to keep the default center with an explicit radius.
...
export component Example inherits Rectangle {
    background: @radial-gradient(circle at 25px 25px size 60px, #f00 0%, #00f 100%);
}
```
