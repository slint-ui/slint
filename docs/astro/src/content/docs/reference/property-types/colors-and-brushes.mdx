...
### @linear-gradient([in color-space] angle, color percentage, color percentage, ...)
...
Optionally, precede the angle with `in <color-space>` to control the color space colors are
interpolated in, matching CSS syntax. Supported spaces are `srgb` (the default), `oklch`, `oklab`,
and `hsl`:

```slint no-test
background: @linear-gradient(in oklch 90deg, #3f87a6 0%, #f69d3c 100%);
```
...
### @radial-gradient([in color-space] circle [radius] [at x y], color percentage, color percentage, ...)

The first parameter to the macro is always `circle` because only circular gradients are supported.
The optional `radius` value sets the circle radius in the element's local coordinate space.
The `radius` value is a length expression.
If omitted, the radius is half of the element's bounding box diagonal.
...
Optionally, precede `circle` with `in <color-space>` to control the color space colors are
interpolated in. See [Linear Gradients](#linear-gradients) for the supported spaces.
...
### @conic-gradient([in color-space] [from angle] [at x y], color angle, color angle, ...)
...
Optionally, precede `from`/`at` with `in <color-space>` to control the color space colors are
interpolated in. See [Linear Gradients](#linear-gradients) for the supported spaces.
