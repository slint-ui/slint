...
Radial gradients are like linear gradients but the colors are interpolated circularly or elliptically instead of
along a line. To describe a radial gradient, use the `@radial-gradient` macro with the following signature:

### @radial-gradient(shape [at x y], color percentage, color percentage, ...)

The `shape` parameter is one of:

- `circle [radius]`: a circle.
  The optional `radius` value sets its radius.
  If omitted, the radius is half of the element's bounding box diagonal.
- `ellipse [radius-x radius-y]`: an ellipse.
  The optional `radius-x` and `radius-y` values set its horizontal and vertical radii.
  If omitted, the radii match CSS `farthest-corner`:
  the ellipse passes through the bounding box corner farthest from the center,
  with the aspect ratio of the distances from the center to the farthest sides.

The radii are length expressions in the element's local coordinate space.
Unlike in CSS, they can't be percentages.
