...
Logical keys represent the the character a key produces on the current keyboard layout.
Physical Keys represent the physical position of a key on the keyboard, which is unaffected by the current keyboard layout.
