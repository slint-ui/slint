...
An image is created with an `@image-url()` image literal, whose image is
decoded when code is generated, or set by the
...
Unlike an import path, the address decodes the string escape sequences
`\"`, `\\`, `\n`, and `\u{...}`,
