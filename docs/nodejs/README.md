...
Astro Starlight site for the Slint Node.js API, with an API section generated by TypeDoc from the `api/js/node` TypeScript sources.
...
To build the native module and this site in one step from the repository root, run `pnpm -C api/js/node run docs`. That is the same pipeline CI uses before publishing under `…/docs/node/`.

When iterating only in `docs/nodejs`, run `pnpm -C api/js/node run build` first whenever the sidebar API reference must match the compiled native module and generated declarations.
