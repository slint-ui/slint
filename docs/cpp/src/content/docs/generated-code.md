...
* A `show` function, which will show the component on the screen.
  You still need to spin the event loop by `slint::run_event_loop()`
...
* A `window` function that provides access to the `slint::Window`,
...
in `slint::ComponentHandle`. This is a smart pointer that owns the
actual instance and keeps it alive as long as at least one
`slint::ComponentHandle` is in scope, similar to `std::shared_ptr<T>`.

For more complex user interfaces it's common to supply data in the form of an
abstract data model, that's used with [`for` - `in`](https://slint.dev/docs/slint/guide/language/coding/repetition-and-data-models/#models)
...
`slint::Model` and you can sub-class it yourself. For convenience,
the `slint::VectorModel` provides an implementation that's backed
by a `std::vector<T>`.
...
You can declare [globally available singletons](https://slint.dev/docs/slint/guide/language/coding/globals/) in your
