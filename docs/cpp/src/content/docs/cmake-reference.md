...
Use the `LIBRARY_PATHS` argument to specify the name and paths to [component libraries](https://slint.dev/docs/slint/guide/language/coding/file/#component-libraries),
...
By default, images from [`@image-url()`](https://slint.dev/docs/slint/reference/primitive-types/#image) or fonts that your Slint files reference are loaded from disk at run-time. This minimizes build times, but requires that the directory structure with the files remains stable. If you want to build a program that runs anywhere, then you can configure the Slint compiler to embed such sources into the binary.
