...
| <SlintRef type="StringType" label="string" /> | `slint::SharedString` | A reference-counted string type that uses UTF-8 encoding and can be easily converted to a `std::string_view` or a `const char *`. |
| <SlintRef type="color" /> | `slint::Color` | |
| <SlintRef type="brush" /> | `slint::Brush` | |
| <SlintRef type="ImageType" label="image" /> | `slint::Image` | |
...
| <SlintRef type="data_transfer" label="data-transfer" /> | `slint::DataTransfer` | Data associated with a drag-drop transfer. |
| <SlintRef type="styled_text" label="styled-text" /> | `slint::StyledText` | Styled text parsed from markdown or plain text. Use `StyledText::from_markdown()` or `StyledText::from_plain_text()` to create. |
| <SlintRef type="Point" /> | `slint::LogicalPosition` | A struct with `x` and `y` fields, representing logical coordinates. |
