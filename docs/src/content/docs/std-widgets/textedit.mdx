...
Similar to [`LineEdit`](/master/docs/slint/std-widgets/lineedit), but can be used to enter several lines of text
...
-   **`clear-focus()`** Call this function to remove keyboard focus from this `TextEdit` if it currently has the focus. See also [focus handling](/master/docs/slint/concepts/focus).
