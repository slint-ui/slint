...
Similar to [`LineEdit`](/tng/std-widgets/lineedit), but can be used to enter several lines of text
...
-   **`clear-focus()`** Call this function to remove keyboard focus from this `TextEdit` if it currently has the focus. See also [focus handling](/tng/guide/focus).
