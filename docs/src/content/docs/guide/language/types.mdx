...
| `brush`   | A complex type that can be a `color` or a `gradient`. See [Colors & Brushes](/master/docs/slint/reference/builtins/colors)   | `transparent` |
| `color`   | Color fill with values of `red`, `green`, `blue`, and `alpha`. See [Colors & Brushes](/master/docs/slint/reference/builtins/colors)   | `transparent` |
