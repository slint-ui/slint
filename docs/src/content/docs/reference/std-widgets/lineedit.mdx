...
A widget used to enter a single line of text. See [`TextEdit`](master/docs/slint/reference/std-widgets/textedit) for
...
```slint no-test 'text: "Initinal text";'
LineEdit {
    text: "Initinal text";
}
```
...
### clear-focus()
Call this function to remove keyboard focus from this `LineEdit` if it currently has the focus. See also [Focus Handling](../concepts/focus.md).
