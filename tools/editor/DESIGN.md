...
Status: implemented.
The one piece of the design still outstanding is the UI split: the editor window's `.slint` sources stay in `internal/editor-preview` until the shared `global Api` is split, as described under "Contents".
...
"Thin" is the goal, not the starting point: the editor binary used to compile the entire language server and formatter, and this crate is what shed that.

It is not a language server and must not look like one internally.
The in-process round trip through `lsp_server::Message` framing that the editor once did is gone, and must not come back.
...
- The binary entry point, the CLI, and the startup/threading code, all in `main.rs`.
- Window setup and editor chrome: creating the editor window, wiring the macOS unified title bar, and driving the `sparklers` auto-updater around the shared engine-start function `preview::run_with_ui`.
...
- The session driver: a tokio loop owning an `EditorSession` (from `internal/editor-preview`) and selecting over file-watcher events, preview messages, and the recompile idle timer.
  File watching uses `i_slint_live_preview::file_watcher`.
...
Moving the binary from `[[example]] slint-editor` in `tools/lsp` to a `[[bin]]` here changed the artifact path
(`…/examples/slint-editor` became `…/slint-editor`) and the manifest paths.
Everything that referred to the old artifact path or manifest now points here:

- Workflows: `visual_editor_windows_msix.yaml`, `visual_editor_macos_dmg.yaml`, `visual_editor_linux_flatpak.yaml` (artifact paths, packaging paths, and the version lookups, which key on package name `slint-editor`).
...
- Project files living with this crate: `dev.slint.VisualEditor.yml`, `macos-project.yml` (binary paths and version lookups inside them).
- `docs/development/macos-visual-editor-dmg.md`, `REUSE.toml` (path-scoped license entries), `.gitignore`.
- `.github/ci_path_filters.yaml`: `tools/editor/**`, otherwise editor-only changes skip CI.
...
- `internal/editor-preview`: everything the editor and the LSP preview share — document model, `EditorSession`, canvas engine, the `LspToPreviews` fan-out, (initially) both windows' UI. Published.
- `tools/lsp`: the language server — LSP request handling, `ServerNotifier` (single copy, private), `lsp_to_editor` client helpers, formatter, host-language search, and all preview transports (embedded, child-process, remote WebSocket, wasm). Published.
