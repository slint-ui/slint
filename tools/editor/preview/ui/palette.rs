...
fn collect_palette_from_globals(
    ...
    for (name, global) in tr.all_elements().iter().filter_map(|(n, e)| match e {
        ...
        let properties = properties::get_properties(
            &i_slint_editor_preview::ElementRcNode { element: global.clone(), debug_index: 0 },
...
mod tests {
    ...
    fn compile(source: &str) -> (i_slint_editor_preview::DocumentCache, lsp_types::Url) {
        let (dc, url, diag) =
            i_slint_editor_preview::test::loaded_document_cache(source.to_string());
    ...
    fn test_std_widgets_palette() {
        ...
        for (style, border) in cases {
            ...
            let mut session = i_slint_editor_preview::test::session_with(
                i_slint_editor_preview::DocumentCache::new(config),
            );
            let (url, _) = i_slint_editor_preview::test::load(
                &mut session,
