...
use i_slint_editor_preview::editing::{import_edit, text_edit};
use i_slint_editor_preview::util;
...
pub fn placeholder() -> String {
    format!(
        ...
        i_slint_editor_preview::NODE_IGNORE_COMMENT
    )
}

#[derive(Clone, Debug)]
pub struct DropInformation {
    pub target_element_node: i_slint_editor_preview::ElementRcNode,
...
pub struct InsertInformation {
    pub insertion_position: i_slint_editor_preview::editing::VersionedPosition,
...
pub fn insert_position_at_end(
    target_element_node: &i_slint_editor_preview::ElementRcNode,
) -> Option<InsertInformation> {
    target_element_node.with_element_node(|node| {
        let closing_brace = i_slint_editor_preview::util::last_non_ws_token(node)?;
        ...
        Some(InsertInformation {
            insertion_position: i_slint_editor_preview::editing::VersionedPosition::new(
...
pub fn add_new_component(
    ...
    Some((
        i_slint_editor_preview::editing::create_workspace_edit_from_path(
            document_cache,
            source_file.path(),
            vec![edit],
        )?,
...
fn drop_target_element_nodes(
    component_instance: &ComponentInstance,
    position: LogicalPoint,
    filter: Box<dyn Fn(&i_slint_editor_preview::ElementRcNode) -> bool>,
) -> Vec<i_slint_editor_preview::ElementRcNode> {
    ...
    for sc in &element_selection::collect_all_element_nodes_covering(position, component_instance) {
        ...
        if en.with_element_node(i_slint_editor_preview::is_element_node_ignored) {
            continue;
        }
...
fn is_recursive_inclusion(
    root_node: &Option<&i_slint_editor_preview::ElementRcNode>,
    component_type: &str,
) -> bool {
...
fn find_filtered_location(
    ...
    position: LogicalPoint,
    filter: Box<dyn Fn(&i_slint_editor_preview::ElementRcNode) -> bool>,
    mark: Box<dyn Fn(&i_slint_editor_preview::ElementRcNode) -> bool>,
    component_type: &str,
) -> Option<DropInformation> {
    ...
    if layout_kind != ui::LayoutKind::None {
        ...
        let children_geometries: Vec<_> = drop_target_node
            .children()
            .iter()
            .filter(|c| !c.with_element_node(i_slint_editor_preview::is_element_node_ignored))
            .filter_map(|c| {
...
fn drop_ignored_elements_from_node(
    node: &i_slint_editor_preview::ElementRcNode,
    ...
    node.with_element_node(|node| {
        node.children()
            .filter_map(|c| {
                let e = i_slint_editor_preview::extract_element(c.clone())?;
                if i_slint_editor_preview::is_element_node_ignored(&e) {
...
fn node_removal_text_edit(
    ...
    node: &SyntaxNode,
    replace_with: String,
) -> Option<i_slint_editor_preview::editing::SingleTextEdit> {
    let range = util::text_range_to_lsp_range(
        &node.source_file.clone(),
        pretty_node_removal_range(node)?,
        document_cache.format,
    );
    i_slint_editor_preview::editing::SingleTextEdit::from_path(
        document_cache,
...
pub fn create_swap_element_workspace_edit(
    ...
    if let Some(component_info) = preview::get_component_info(&component_type) {
            ...
            import_edit::create_import_edit(doc, &component_type, &import_file, format)
        {
            ...
            edits.push(i_slint_editor_preview::editing::SingleTextEdit::from_path(
                &document_cache,
                source_file.path(),
                edit,
            )?);
...
mod tests {
    ...
    use i_slint_editor_preview::{editing::text_edit, test, util};
...
"#;
    ...
    ) -> (i_slint_editor_preview::DocumentCache, lsp_types::WorkspaceEdit) {
    ...
    fn test_extend_with_new_properties_keeps_existing_text_default() {
        let default_properties = vec![i_slint_editor_preview::editing::PropertyChange::new(
            "text",
            "\"Text\"".to_string(),
        )];
        let mut extra_properties = vec![
            i_slint_editor_preview::editing::PropertyChange::new("x", "20px".to_string()),
            i_slint_editor_preview::editing::PropertyChange::new("y", "24px".to_string()),
        ];
