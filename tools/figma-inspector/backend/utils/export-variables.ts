...
function createReferenceExpression(
    ...
    const isCircularReference =
        commonParts >= 2 && currentParts.length >= 3 && targetParts.length >= 3;

    if (isCircularReference) {
        console.warn(
            `Detected circular reference: ${currentPath} -> ${targetPath.row}`,
        );
        ...
        try {
            const targetCollection = collectionStructure.get(
    ...
    if (!targetCollection) {
        console.warn(
            `Collection not found: ${targetPath.collection}`,
    ...
    if (targetModes.length === 0) {
        console.warn(
            `No modes found in target collection: ${targetPath.collection}`,
        );
        return { value: null };
    }
    ...
    if (!targetCollection.variables.has(targetPath.row)) {
        console.warn(
            `Variable row ${targetPath.row} not found in collection ${targetPath.collection}`,
        );
        console.error(
            `Variable not found: ${targetPath.row} in collection ${targetCollection.formattedName}`,
        );
        console.log(
            `Available variables:`,
            Array.from(targetCollection.variables.keys()).join(", "),
        );
        return { value: null };
    }

    // First try: exact match with sanitized names
    let targetModeName = targetModes.find(
        (mode) =>
    ...
    if (!targetModeName) {
        ...
        console.log(
            `Using default mode ${targetModeName} for reference to ${referenceId}`,
        );
    }
...
> {
    ...
    try {
        ...
        for (const [
            ...
            function generateStructsAndInstances(
                    ...
                    for (const [
                        ...
                        if (childNode.children.size > 0) {
                            ...
                            if (collectionData.modes.size <= 1) {
                                ...
                                if (!valueToUse) {
                                    console.error(
                                        `No value for nested property ${sanitizedChildName}`,
                                    );
                                    result += `${indent}${sanitizedChildName}: ${childNode.type === "COLOR" ? "#808080" : "0px"},\n`;
                                } else if (valueToUse.startsWith("@ref:")) {
                                    console.warn(
                                        `Unresolved reference for nested property ${sanitizedChildName}: ${valueToUse}`,
                                    );
                                    result += `${indent}${sanitizedChildName}: ${childNode.type === "COLOR" ? "#808080" : "0px"},\n`;
                                } else {
                ...
                function generateProperty(
                    ...
                    if (collectionData.modes.size > 1) {
                        ...
                        console.log(
                            `Processing single-mode for ${node.name} with ${node.valuesByMode.size} entries`,
                        );

                        // Directly get the first (and only) mode entry
                        const firstModeEntry = Array.from(
                            node.valuesByMode.entries(),
                        )[0];
                        ...
                        console.log(
                            `Mode ${modeName} has value: ${modeData.value}`,
                        );
                        ...
                        if (modeData.value.startsWith("@ref:")) {
                            console.warn(
                                `Unresolved reference for ${node.name}: ${modeData.value}`,
                            );
                            return `${indent}out property <${slintType}> ${node.name}: ${slintType === "brush" ? "#808080" : "0px"};\n`;
                        }
            ...
            if (collectionData.modes.size > 1) {
                const sanitizedName = sanitizeEnumName(
                    collectionData.formattedName,
                );
...
function generateSchemeStructs(
    ...
    const schemeVariables = new Map<
        string,
        { type: string; path: string[]; isStruct: boolean }
    >();
            ...
            if (childNode.children.size > 0) {
                ...
                if (hasLeafDescendants > 0) {
                    ...
                    for (const [
                        gcName,
                        gcNode,
                    ] of childNode.children.entries()) {
                        if (gcNode.valuesByMode) {
                            // Add as a field to our struct
                            const fieldType = getSlintType(
                                gcNode.type || "COLOR",
                            );
                            schemeStructs
                                .get(structName)!
                                .fields.set(gcName, fieldType);
                        }
                    }
                }
            } else if (childNode.valuesByMode) {
