...
/// The extension of the map the compiler writes next to the generated code.
pub const MAP_EXTENSION: &str = "slintcov";
...
/// A point's record, `<kind> <name> <span> <path>`; a branch's name is its
/// operator and the outcome, `? true`.
fn parse_record(record: &str) -> Option<Point> {
    let (kind, rest) = record.split_once(' ')?;
    let (name, rest) = rest.split_once(' ')?;
    let (kind, rest) = match kind {
        "element" => (Kind::Element, rest),
...
/// as JSON, of the given source files alone when any are given.
pub fn export_coverage(
    ...
    if !sources.is_empty() {
        export.arg("-sources").args(sources);
    }
...
"#;
    ...
    fn record_with_spaces_in_path() {
        ...
        assert_eq!((point.kind, point.line, point.column), (Kind::Element, 7, 36));
        assert_eq!(point.name, "Window");
        assert_eq!(point.file, Path::new("/my dir/a.slint"));
        let point = parse_record("branch && false 7:36-7:38 /a.slint").unwrap();
        assert_eq!((point.kind, point.name.as_str()), (Kind::Branch { outcome: false }, "&&"));
        let point = parse_record("binding level 7:36-7:43 /a.slint").unwrap();
        assert_eq!((point.kind, point.name.as_str()), (Kind::Binding, "level"));
