...
pub fn check(source: &str, case: &Path, report: &Report) -> Result<(), String> {
    ...
    for (i, line) in source.lines().enumerate() {
        if is_caret_line(line) {
            carets(line).map_err(|e| format!("line {}: {e}", i + 1))?;
        }
    }
...
/// The carets of a caret line, each as the column of its `^` and its marks.
fn carets(line: &str) -> Result<Vec<(usize, &str)>, String> {
    let rest = line.strip_prefix(OPENER).ok_or("a caret line starts in the first column")?;
...
fn annotate(
    source: &str,
    lines: &BTreeMap<usize, Vec<(usize, String, bool)>>,
...
/// The caret line stating the points measured on a line: the two outcomes of
/// a decision, listed one after the other, make one caret.
fn caret_line(entries: &[(usize, String, bool)]) -> Result<String, String> {
    let status = |reached: bool| if reached { '+' } else { '-' };
    let mut out = String::from(OPENER);
    let mut entries = entries.iter();
    while let Some((column, what, reached)) = entries.next() {
        let mut marks = status(*reached).to_string();
        if what.starts_with("branch ") {
            let (_, _, false_reached) =
                entries.next().expect("the false outcome follows the true one");
            marks.push(status(*false_reached));
        }
        if *column <= out.len() {
            return Err(format!(
...
mod tests {
    ...
    fn report(level: usize, touch: usize) -> Report {
        ...
        add(&mut report, "binding level", level, 31, 4);
        add(&mut report, "branch && true", level, 33, 4);
        add(&mut report, "branch && false", level, 33, 0);
        add(&mut report, "branch ? true", level, 38, 2);
        add(&mut report, "branch ? false", level, 38, 2);
        add(&mut report, "element TouchArea", touch, 5, 1);
        add(&mut report, "handler clicked", touch, 17, 0);
        add(&mut report, "call pressed", touch, 30, 0);
        report.add(
            &Point {
                kind: "element".into(),
                name: Some("Led".into()),
...
";
    ...
    fn updating() {
        ...
        add(&mut report, "element Rectangle", 5, 1, 1);
