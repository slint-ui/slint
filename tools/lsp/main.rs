...
async fn main() {
    ...
    if let Some(command) = &args.command {
        ...
        if let Err(error) = run_lsp_server(args).await {
...
async fn run_lsp_server(args: Cli) -> async_lsp::Result<()> {
    let (server, _) = async_lsp::MainLoop::new_server(move |client| {
        ...
        router.request::<Initialize, _>(move |ctx, params| {
            let document_cache = ctx.get().unwrap().document_cache.clone();
            let client = inner_client.clone();
            async move {
                let preview_config =
                    startup_lsp(&params, &client, document_cache).await.map_err(|err| {
                        async_lsp::ResponseError::new(async_lsp::ErrorCode::INTERNAL_ERROR, err)
                    })?;
                client.emit(SetContextEvent(params, preview_config));
                Ok(InitializeResult::default())
            }
        });
        let inner_client = client.clone();
        router.event::<SetContextEvent>(move |ctx, params| {
            let new_ctx = create_context(inner_client.clone(), params.0, args.clone(), params.1);
            ctx.set(new_ctx);
            std::ops::ControlFlow::Continue(())
        });
        router.event::<SendDiagnosticsEvent>(move |ctx, params| {
            let server_notifier = ctx.get().unwrap().server_notifier.clone();
            tokio::task::spawn_local(ctx.get().unwrap().document_cache.clone().exec(
                move |document_cache| {
                    send_diagnostics(
                        server_notifier,
                        document_cache,
...
fn register_notifications(router: &mut Router<OnceLock<Context>>) {
    ...
    router.notification::<DidCloseTextDocument>(|ctx, params| {
        if let Err(err) = close_document(ctx.get_mut().unwrap(), params.text_document.uri) {
    ...
    router.notification::<DidChangeConfiguration>(|ctx, params| {
        let mut ctx = ctx.get_mut().unwrap();
        let client = ctx.server_notifier.client.clone();
        let document_cache = ctx.document_cache.clone();
        tokio::task::spawn_local(async move {
            let preview_config = load_configuration(&params.settings, client, document_cache).await;
        });
        std::ops::ControlFlow::Continue(())
    });
    router.notification::<DidChangeWatchedFiles>(|ctx, params| {
        ...
        for fe in params.changes {
            ...
            if let Err(err) = trigger_file_watcher(ctx, fe.uri, fe.typ) {
                return std::ops::ControlFlow::Break(Err(async_lsp::Error::Response(
                    async_lsp::ResponseError::new(async_lsp::ErrorCode::INTERNAL_ERROR, err),
                )));
            }
        }
    ...
    router.notification::<common::PreviewToLspMessage>(|ctx, params| {
        handle_preview_to_lsp_message(params, ctx.get_mut().unwrap());
        std::ops::ControlFlow::Continue(())
    });
}
