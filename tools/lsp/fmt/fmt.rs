...
mod tests {
    ...
    #[track_caller]
    fn assert_formatting(unformatted: &str, legacy_formatted: &str) {
        let formatter = Formatter::new().expect("formatter should initialize");
        let formatted_from_unformatted =
            formatter.format_str(unformatted).expect("formatting should succeed").text;
        let second_pass = formatter
            .format_str(&formatted_from_unformatted)
            .expect("formatting should succeed")
            .text;
        assert_eq!(second_pass, formatted_from_unformatted);

        let formatted_from_legacy =
            formatter.format_str(legacy_formatted).expect("formatting should succeed").text;
        let legacy_second_pass =
            formatter.format_str(&formatted_from_legacy).expect("formatting should succeed").text;
        assert_eq!(legacy_second_pass, formatted_from_legacy);
    }
