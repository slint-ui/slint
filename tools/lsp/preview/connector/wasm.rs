...
impl common::PreviewToLsp for WasmPreviewToLsp {
    fn send(&self, message: &common::PreviewToLspMessage) -> common::Result<()> {
        WASM_CALLBACKS.with_borrow(|callbacks| {
            let notifier = js_sys::Function::from((callbacks.as_ref().expect("Callbacks were set up earlier").lsp_notifier).clone());
            let value = serde_wasm_bindgen::to_value(&message)?;
            notifier.call1(&JsValue::UNDEFINED, &value).map_err(|_| "Failed to send message to LSP".to_string())?;
            Ok(())
        })
    }
}
