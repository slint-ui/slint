...
fn create_populate_command(
    uri: lsp_types::Url,
    version: i_slint_compiler::diagnostics::SourceFileVersion,
    title: String,
    text: String,
) -> Command {
...
fn populate_command_range(node: &SyntaxNode) -> Option<lsp_types::Range> {
    ...
    eprintln!("    Populate range: {start_offset:?} - {end_offset:?}");

    (start_offset <= end_offset)
        .then_some(util::map_range(&node.source_file, TextRange::new(start_offset, end_offset)))
}
        ...
        if let Some(source_version) = text_document.version {
            if let Some(current_version) = version {
                if current_version != source_version {
                    return Err(LspError {
                        code: LspErrorCode::InvalidParameter,
                        message: "Document version mismatch. Please refresh your command data"
                            .into(),
                    });
                }
            } else {
...
fn get_code_lenses(
    ...
    if let Some(node) = &doc.node {
        eprintln!("Code lenses: Have a document");
        let has_non_ws_token = node.children_with_tokens().any(|nt| {
            eprintln!("   token or node of kind: {:?}", nt.kind());
            nt.kind() != SyntaxKind::Whitespace && nt.kind() != SyntaxKind::Eof
        });
        if !has_non_ws_token {
            eprintln!("Code lenses: Have a document, without any contents");
            if let Some(range) = populate_command_range(&node) {
                eprintln!("Code lenses: Have a document, without any contents, range: {range:?}");
