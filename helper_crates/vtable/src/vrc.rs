...
impl<VTable: VTableMetaDropInPlace, X: HasStaticVTable<VTable>> VRc<VTable, X> {
    ...
    pub fn new(data: X) -> Self {
        // Safety: `STATIC_VTABLE` is 'static, so it outlives every VRc.
        unsafe { Self::new_with_vtable(X::STATIC_VTABLE, data) }
    }
}

impl<VTable: VTableMetaDropInPlace + 'static, X> VRc<VTable, X> {
    /// Same as [`VRc::new`], but with an explicitly provided vtable instead of
    /// the one associated with `X`'s type through [`HasStaticVTable`].
    ///
    /// This is what a producer that describes its instances at runtime needs:
    /// the description is then chosen per instance rather than per Rust type,
    /// and can live in storage the producer owns (a compilation unit, say)
    /// instead of in a `static`.
    ///
    /// # Safety
    ///
    /// `vtable` must describe `data`, and must stay alive and immutable for as
    /// long as any [`VRc`] or [`VWeak`] derived from the returned value — the
    /// vtable is still reached after `data` itself has been dropped, to
    /// deallocate. Keeping the vtable's owner alive through `data` discharges
    /// this.
    pub unsafe fn new_with_vtable(vtable: &VTable::VTable, data: X) -> Self {
        // The lifetime is erased into the allocation; the caller's obligation
        // above is what stands in for it.
        let vtable: &'static VTable::VTable = unsafe { core::mem::transmute(vtable) };
