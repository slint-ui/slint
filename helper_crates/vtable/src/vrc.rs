...
impl<VTable: VTableMetaDropInPlace, X: HasStaticVTable<VTable>> VRc<VTable, X> {
    fn alloc_inner<'vt>() -> NonNull<VRcInner<'vt, VTable, X>> {
    ...
    pub fn new(data: X) -> Self {
        ...
        unsafe {
            ...
            (*mem).data_offset =
                (&raw const (*mem).data as usize - mem as *const _ as usize) as u16;
            VRc { inner }
        }
    }

    /// Constructs a new VRc while giving you a VWeak to the allocation, to allow you to construct a data which holds a weak pointer to itself.
    ///
    /// Similar to [`Rc::new_cyclic()`](core::rc::Rc::new_cyclic) from std.
    pub fn new_cyclic(data_fn: impl FnOnce(&VWeak<VTable, X>) -> X) -> Self {
        let inner = Self::alloc_inner();
        let mem = inner.as_ptr();

        unsafe {
            (&raw mut (*mem).vtable).write(X::static_vtable());
            (&raw mut (*mem).strong_ref).write(AtomicU32::new(0));
            (&raw mut (*mem).weak_ref).write(AtomicU32::new(2));
            (&raw mut (*mem).data_offset)
                .write((&raw const (*mem).data as usize - mem as *const _ as usize) as u16);
            let weak = VWeak { inner: Some(inner) };
            (&raw mut (*mem).data).write(data_fn(&weak));
            (*mem).strong_ref = AtomicU32::new(1);
            VRc { inner }
        }
    }
