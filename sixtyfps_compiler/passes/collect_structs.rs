...
pub fn collect_structs(root_component: &Rc<Component>, _diag: &mut BuildDiagnostics) {
    ...
    let mut maybe_collect_object = |ty: &Type| {
        if let Type::Object { name: Some(name), .. } = ty {
            hash.entry(name.clone()).or_insert_with(|| ty.clone());
        };
    };
    ...
    *used_struct = Vec::with_capacity(hash.len());
    while let Some(next) = hash.iter().next() {
        // Here, using BTreeMap::poo_first would be great when it is stable
...
fn sort_struct(hash: &mut BTreeMap<String, Type>, vec: &mut Vec<Type>, key: &str) {
    ...
    if let Type::Object { fields, .. } = &ty {
        for (_, sub_ty) in fields {
            visit_named_object(&sub_ty, hash, vec);
        }
    }
    vec.push(ty)
}

fn visit_named_object(ty: &Type, hash: &mut BTreeMap<String, Type>, vec: &mut Vec<Type>) {
    match ty {
        Type::Object { name: Some(name), .. } => sort_struct(hash, vec, &name),
        Type::Object { fields, name: None } => {
            for (_, sub_ty) in fields {
                visit_named_object(&sub_ty, hash, vec);
            }
        }
        Type::Array(x) => visit_named_object(&x, hash, vec),
        _ => {}
    }
}
