...
To use the components in your existing project, download the <a href={`${import.meta.env.BASE_URL}zip/material-1.0.1.zip`}>material component source</a> and extract it into your repository.
    ...
    let config = slint_build::CompilerConfiguration::new().with_library_paths(
        std::collections::HashMap::from([(
            ...
            std::path::Path::new(&std::env::var_os("CARGO_MANIFEST_DIR").unwrap())
                .join("material-1.0.1/material.slint"),
        )]),
    );
...
let ui = slint.loadFile("ui/main.slint", {
    libraryPaths: {
        "material": path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "material-1.0.1", "material.slint")
    }
});
```
</TabItem>
...
ui = slint.load_file(
    ...
    library_paths={
        "material": Path(__file__).parent / "material-1.0.1" / "material.slint"
    },
)
```
</TabItem>
