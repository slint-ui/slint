...
A `CheckBoxTile` is a control that combines a text label with a `CheckBox`. See <a href={`${import.meta.env.BASE_URL}components/checkboxes/check_box/`}>CheckBox</a> and <a href={`${import.meta.env.BASE_URL}components/list_tile/`}>ListTile</a> for more information.
