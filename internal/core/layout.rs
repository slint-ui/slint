...
impl GridLayoutOrganizedData {
    ...
    fn col_or_row_and_span(
        ...
        // Stride-based indirection for repeated items:
...
/// Stride-based organized data generator for grid layouts with repeaters.
...
impl<'a> OrganizedDataGenerator<'a> {
    fn new(
        ...
        // Calculate total dynamic u16 entries
        let total_dynamic_u16: usize = repeater_indices
            .chunks(2)
            .enumerate()
            .map(|(i, chunk)| {
                let count = chunk.get(1).copied().unwrap_or(0) as usize;
                let step = repeater_steps.get(i).copied().unwrap_or(1) as usize;
                count * step * 4 // data entries only
            })
            .sum();
        assert!(result.len() >= total_dynamic_u16);
        let repeat_u16_offset = result.len() - total_dynamic_u16;
        Self {
            repeater_indices,
            repeater_steps,
            counter: 0,
    ...
    fn add(&mut self, col: u16, colspan: u16, row: u16, rowspan: u16) {
        ...
        loop {
            if let Some(nr) = self.repeater_indices.get(self.next_rep * 2) {
                ...
                if nr == self.counter {
                    // First cell of this repeater — write jump cell [data_base, stride, 0, 0]
                    let data_u16_start = self.repeat_u16_offset;
                    let stride = step * 4;

                    // Write 1 jump cell (4 u16): [data_base, stride, 0, 0]
                    res[self.current_offset * 4] = data_u16_start as _;
                    res[self.current_offset * 4 + 1] = stride as _;
                    res[self.current_offset * 4 + 2] = 0;
                    res[self.current_offset * 4 + 3] = 0;
...
fn organize_grid_layout_impl(
    ...
    let mut organized_data = GridLayoutOrganizedData::default();
    // Stride format: static_cells * 4 + num_repeaters * 4 (jump cells)
    ...
    let dynamic_u16: usize = repeater_indices
        .chunks(2)
        .enumerate()
        .map(|(i, chunk)| {
            let count = chunk.get(1).copied().unwrap_or(0) as usize;
        ...
        .sum();
    organized_data.resize((static_cells + num_repeaters) * 4 + dynamic_u16, 0 as _);
...
/// Standard cache: cache[cache[index] + ri * 2] (2 entries per item: pos, size)
...
/// Stride-based layout cache generator for grid layouts with repeaters.
...
impl<'a> GridLayoutCacheGenerator<'a> {
    fn new(
        ...
        // Calculate total dynamic f32 entries
        let total_dynamic_f32: usize = repeater_indices
            .chunks(2)
            .enumerate()
            .map(|(i, chunk)| {
                let count = chunk.get(1).copied().unwrap_or(0) as usize;
                let step = repeater_steps.get(i).copied().unwrap_or(1) as usize;
                count * step * 2 // data entries only
            })
            .sum();
        assert!(result.len() >= total_dynamic_f32);
        let repeat_f32_offset = result.len() - total_dynamic_f32;
        Self {
            repeater_indices,
            repeater_steps,
            counter: 0,
    ...
    fn add(&mut self, pos: Coord, size: Coord) {
        ...
        loop {
            if let Some(nr) = self.repeater_indices.get(self.next_rep * 2) {
                ...
                if nr == self.counter {
                    // First cell of this repeater — write jump cell [data_base, stride]
...
pub fn solve_grid_layout(
    ...
    let num_repeaters = repeater_indices.len() / 2;
    result.resize(2 * constraints.len() + num_repeaters * 2, 0 as _);
    let mut generator =
        GridLayoutCacheGenerator::new(&repeater_indices, &repeater_steps, &mut result);
...
mod tests {
    ...
    fn test_organized_data_generator_1_fixed_cell_1_repeater() {
        ...
        // Stride format: (1 static + 1 jump) * 4 + 3*1*4 data = 8 + 12 = 20
        let mut result = GridLayoutOrganizedData::default();
        let num_cells = 4;
        let repeater_indices = &[1u32, 3u32];
        result.resize(20, 0 as _);
        let mut generator = OrganizedDataGenerator::new(repeater_indices, &[], &mut result);
        generator.add(0, 1, 0, 2); // fixed
        generator.add(1, 2, 1, 3); // repeated row 0
        generator.add(1, 1, 2, 4); // repeated row 1
        generator.add(2, 2, 3, 5); // repeated row 2
        assert_eq!(
            result.as_slice(),
            &[
                0, 1, 0, 2, // fixed cell
    ...
    fn test_organize_data_repeated_rows() {
        ...
        // Now test GridLayoutCacheGenerator (stride-based format)
        let mut layout_cache_v = SharedVector::<Coord>::default();
        // Stride: 1 jump cell (2 f32) + 6 data cells (12 f32) = 14
        layout_cache_v.resize((1 * 2 + num_columns * num_rows * 2) as usize, 0 as _);
        ...
        assert_eq!(
            ...
            &[
                2., 4., // jump cell: data at f32 pos 2, stride=4 (=step*2=2*2)
                0., 50., 0., 50., // row 0 data
                50., 50., 50., 50., // row 1 data
                100., 50., 100., 50., // row 2 data
            ]
        );

        // Stride access: cache[cache[jump_index] + ri * stride + child_offset]
        let layout_cache_v_stride_access = |jump_index: usize,
        ...
        assert_eq!(layout_cache_v_stride_access(0, 0, 4, 0), 0.);
        assert_eq!(layout_cache_v_stride_access(0, 1, 4, 0), 50.);
        assert_eq!(layout_cache_v_stride_access(0, 2, 4, 0), 100.);
        // Y pos for child 1 (child_offset=2)
        assert_eq!(layout_cache_v_stride_access(0, 0, 4, 2), 0.);
        assert_eq!(layout_cache_v_stride_access(0, 1, 4, 2), 50.);
        assert_eq!(layout_cache_v_stride_access(0, 2, 4, 2), 100.);
    }
    ...
    fn test_organize_data_repeated_rows_multiple_repeaters() {
        ...
        assert_eq!(
            organized_data.as_slice(),
            &[
                8, 8, 0, 0, // rep 0 jump: data at 8, stride=8 (=step*4=2*4)
                32, 12, 0, 0, // rep 1 jump: data at 32, stride=12 (=step*4=3*4)
                // Rep 0 data
                0, 1, 0, 1, 1, 1, 0, 1, // row 0: col 0, col 1
                0, 1, 1, 1, 1, 1, 1, 1, // row 1: col 0, col 1
                0, 1, 2, 1, 1, 1, 2, 1, // row 2: col 0, col 1
        ...
        // Now test GridLayoutCacheGenerator (stride-based format)
        let mut layout_cache_v = SharedVector::<Coord>::default();
        // Stride: 2 jump cells (4 f32) + 12 data cells (24 f32) = 28
        layout_cache_v.resize((2 * 2 + (3 * 2 + 2 * 3) * 2) as usize, 0 as _);
        let mut generator = GridLayoutCacheGenerator::new(
            repeater_indices.as_slice(),
            repeater_steps.as_slice(),
            &mut layout_cache_v,
        ...
        assert_eq!(
            layout_cache_v.as_slice(),
            &[
                4., 4., // rep 0 jump: data at pos 4, stride=4 (=step*2=2*2)
                16., 6., // rep 1 jump: data at pos 16, stride=6 (=step*2=3*2)
                0., 50., 0., 50., // rep 0 row 0 data
                50., 50., 50., 50., // rep 0 row 1 data
                100., 50., 100., 50., // rep 0 row 2 data
                150., 50., 150., 50., 150., 50., // rep 1 row 3 data
                200., 50., 200., 50., 200., 50., // rep 1 row 4 data
            ]
        );
        ...
        assert_eq!(layout_cache_v_stride_access(2, 0, 6, 0), 150.);
        assert_eq!(layout_cache_v_stride_access(2, 1, 6, 0), 200.);
        // Repeater 1: Y pos for child 2 (child_offset=4), jump at index 2, stride=6
        assert_eq!(layout_cache_v_stride_access(2, 0, 6, 4), 150.);
        assert_eq!(layout_cache_v_stride_access(2, 1, 6, 4), 200.);
    }
    ...
    fn test_layout_cache_generator_1_fixed_cell_1_repeater() {
        // 4 cells: 1 fixed cell, 1 repeater with step=1, count=3 (standard cache for box layout)
        let mut result = SharedVector::<Coord>::default();
        let repeater_indices = &[1_u32, 3];
    ...
    fn test_layout_cache_generator_4_repeaters() {
        ...
        let mut result = SharedVector::<Coord>::default();
        let repeater_indices = &[1_u32, 0, 1, 4, 6, 2, 8, 0];
        result.resize(8 * 2 + repeater_indices.len(), 0 as _);
        let mut generator = LayoutCacheGenerator::new(repeater_indices, &mut result);
        generator.add(0., 50.); // fixed
        generator.add(80., 10.); // repeated (rep1)
        generator.add(160., 10.);
        generator.add(240., 10.);
        generator.add(320., 10.); // end of rep1
        generator.add(400., 80.); // fixed
        generator.add(500., 20.); // repeated (rep2)
        generator.add(600., 20.); // end of rep2
        assert_eq!(
            result.as_slice(),
            &[
                0., 50., // fixed
