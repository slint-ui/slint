...
fn compute_dirty_regions_recursive<T: ItemRendererFeatures>(
    ...
    let mut child_visitor = |child_component: &ItemTreeRc,
     ...
     -> VisitChildrenResult {
        ...
        // The region the item covers on screen at its current position. It is merged
        // into the owning tree's entry in `tree_screen_rects` so that destroying the
        // tree can repaint that region (see `PartialRenderingState::free_graphics_resources`),
        // and reused below both as the item's current-position dirty rect and, unless
        // the geometry changed, as `own_screen_rect`.
        let new_screen_rect =
            screen_rect_for(new_geom.bounding_rect(), state.transform_to_screen, &state.clipped);
        let new_screen_rect_or_empty = new_screen_rect.unwrap_or_default();
...

        {
            ...
            let tree_key =
                vtable::VRef::as_ptr(crate::item_tree::ItemTreeRc::borrow(child_component))
                    .as_ptr() as usize;
            if child_index == 0
            ...
            match rendering_data.get_entry(&mut cache_ref) {
                Some(PartialRenderingCachedData { data: cached_geom, tracker, .. }) => {
                    ...
                    if geometry_changed {
                        ...
                        dirty_region.add_rect(new_screen_rect_or_empty);

                        new_state
                            .adjust_transforms_for_child(&new_geom.transform(), &old_transform);

                        own_screen_rect = new_screen_rect;
