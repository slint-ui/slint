...
impl<T: InterpolatedPropertyValue + Clone> PropertyValueAnimationData<T> {
    ...
    pub fn compute_interpolated_value(&mut self) -> (T, bool) {
        ...
        if !self.details.enabled || crate::animations::reduced_motion() {
...
mod animation_tests {
    ...
    /// Sets this thread's reduced-motion flag for the duration of a test.
    struct ReducedMotion;

    impl ReducedMotion {
        fn set(reduced: bool) -> Self {
            crate::animations::CURRENT_ANIMATION_DRIVER
    ...
    impl Drop for ReducedMotion {
        fn drop(&mut self) {
            crate::animations::CURRENT_ANIMATION_DRIVER
                .with(|driver| driver.set_reduced_motion(false));
        }
    }

    #[test]
    fn reduced_motion_jumps_to_the_target() {
        let _reduced = ReducedMotion::set(true);
        let compo = Component::new_test_component();
        ...
        set_animated_value(&compo.width, 200, animation_details);
        // Neither the delay nor the duration is waited for.
        assert_eq!(get_prop_value(&compo.width), 200);
        assert_eq!(get_prop_value(&compo.width_times_two), 400);

        // The animation finished, so the binding is gone.
        compo.width.handle.access(|binding| assert!(binding.is_none()));
    }

    #[test]
    fn reduced_motion_jumps_a_spring_to_the_target() {
        let _reduced = ReducedMotion::set(true);
        let compo = Component::new_test_component();

        let spring_details = PropertyAnimation {
            duration: 1000,
            easing: crate::animations::EasingCurve::Spring(0.5),
            ..PropertyAnimation::default()
        };

        compo.width.set(0);
        set_animated_value(&compo.width, 1000, spring_details);
        assert_eq!(get_prop_value(&compo.width), 1000);
        compo.width.handle.access(|binding| assert!(binding.is_none()));
    }

    #[test]
    fn reduced_motion_applies_to_animated_bindings() {
        let compo = Component::new_test_component();
    ...
    #[test]
    fn reduced_motion_finishes_a_running_animation_on_the_next_tick() {
        let compo = Component::new_test_component();
        ...
        // The setting changes while the animation runs: the next frame lands on the target.
        let _reduced = ReducedMotion::set(true);
        crate::animations::CURRENT_ANIMATION_DRIVER.with(|driver| {
            driver
                .update_animations(start_time + DURATION / 2 + core::time::Duration::from_millis(1))
        });
        assert_eq!(get_prop_value(&compo.width), 200);
        assert_eq!(get_prop_value(&compo.width_times_two), 400);
        compo.width.handle.access(|binding| assert!(binding.is_none()));
    ...
    #[test]
    fn clearing_reduced_motion_animates_again() {
        let compo = Component::new_test_component();
        ...
        {
            let _reduced = ReducedMotion::set(true);
            set_animated_value(&compo.width, 200, animation_details.clone());
            assert_eq!(get_prop_value(&compo.width), 200);
        }

        let start_time = crate::animations::current_tick();
        set_animated_value(&compo.width, 300, animation_details);
        assert_eq!(get_prop_value(&compo.width), 200);
