...
pub(crate) mod ffi {
    ...
    #[unsafe(no_mangle)]
    pub unsafe extern "C" fn slint_shared_string_contains(
        self_: &SharedString,
        pat: &SharedString,
    ) -> bool {
        self_.contains(pat.as_str())
    }
    #[test]
    fn test_slint_shared_string_contains() {
        let s = SharedString::from("Hello");
        let pat = SharedString::from("ll");
        unsafe {
            assert!(slint_shared_string_contains(&s, &pat));
        }
    }

    #[unsafe(no_mangle)]
    pub unsafe extern "C" fn slint_shared_string_is_ascii(self_: &SharedString) -> bool {
        self_.is_ascii()
    }
    #[test]
    fn test_slint_shared_string_is_ascii() {
        let s = SharedString::from("Hello");
        unsafe {
            assert!(slint_shared_string_is_ascii(&s));
        }
    }

    #[unsafe(no_mangle)]
    pub unsafe extern "C" fn slint_shared_string_repeat(
        self_: &SharedString,
        n: usize,
        out: &mut SharedString,
    ) {
        *out = SharedString::from(self_.repeat(n));
    }
    #[test]
    fn test_slint_shared_string_repeat() {
        let s = SharedString::from("Hello");
        let mut out = SharedString::default();
        unsafe {
            slint_shared_string_repeat(&s, 3, &mut out);
        }
        assert_eq!(out.as_str(), "HelloHelloHello");
    }
    ...
    pub unsafe extern "C" fn slint_shared_string_replace(
        ...
        *out = SharedString::from(self_.replace(from.as_str(), to.as_str()));
    }
    #[test]
    fn test_slint_shared_string_replace() {
        let s = SharedString::from("Hello, World!");
        let from = SharedString::from("World");
        let to = SharedString::from("Slint");
        let mut out = SharedString::default();
        unsafe {
            slint_shared_string_replace(&s, &from, &to, &mut out);
        }
        assert_eq!(out.as_str(), "Hello, Slint!");
    }

    #[unsafe(no_mangle)]
    pub unsafe extern "C" fn slint_shared_string_trim(
        self_: &SharedString,
        out: &mut SharedString,
    ) {
        *out = SharedString::from(self_.trim());
    }
    #[test]
    fn test_slint_shared_string_trim() {
        let s = SharedString::from("Hello\n");
        let mut out = SharedString::default();
        unsafe {
            slint_shared_string_trim(&s, &mut out);
        }
        assert_eq!(out.as_str(), "Hello");
    }
}
