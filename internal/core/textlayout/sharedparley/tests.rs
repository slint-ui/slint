...
/// Returns the horizontal snap correction and Parley line offset for right-aligned text.
...
fn test_pixel_snap_alignment_cancels_the_origin_snap_delta() {
    // Right alignment cancels the full displacement, independent of the box width.
    ...
    // The correction must leave Parley alignment unchanged.
...
/// A fractional width must not shift text when the origin is already pixel-aligned (#6739).
#[test]
...
/// Origin snapping must not change line breaks (#6739).
/// Sweep fractional widths to cover wrapping boundaries without depending on exact font metrics.
#[test]
fn test_pixel_snap_alignment_never_moves_the_wrap_boundary() {
    ...
    // Ensure the sweep exercised a nonzero correction.
    assert!(saw_a_snap_correction);
}

/// Hit-testing a reported cursor position must return the same byte offset with a nonzero snap correction.
#[test]
fn test_pixel_snap_alignment_x_offset_round_trips_through_hit_testing() {
    ...
    // A zero correction would hide a sign error in the round trip.
