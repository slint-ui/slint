...
///
/// If `byte_offset` falls in the middle of a UTF-8 character or beyond the
/// string, it is rounded up to the next character boundary (via
/// [`ceil_byte_offset`]). In debug builds an assertion fires for such inputs,
/// since callers are expected to hold valid byte offsets (e.g. from
...
pub fn byte_offset_to_utf16_offset(text: &str, byte_offset: usize) -> usize {
    ...
    let byte_offset = ceil_byte_offset(text, byte_offset);
