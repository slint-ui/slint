...
impl SoftwareRenderer {
    ...
    fn render_buffer_impl(
        ...
        let pixel_stride = buffer.line_slice(0).len();
        let num_lines = buffer.num_lines();
        let buffer_pixel_count = num_lines * pixel_stride;
            ...
            window_inner.window_item().as_ref().map(|item| item.as_pin_ref())
        {
            ...
            (euclid::size2(num_lines as _, pixel_stride as _), Brush::default())
        } else {
            (euclid::size2(pixel_stride as _, num_lines as _), Brush::default())
        };
        if size.is_empty() {
            return Default::default();
        }
        assert!(
            if rotation.is_transpose() {
                pixel_stride >= size.height as usize && buffer_pixel_count >= (size.width as usize * pixel_stride + size.height as usize) - pixel_stride
            } else {
                pixel_stride >= size.width as usize && buffer_pixel_count >= (size.height as usize * pixel_stride + size.width as usize) - pixel_stride
            },
            "buffer of size {} with stride {pixel_stride} is too small to handle a window of size {size:?}", buffer_pixel_count
        );
