...
pub mod api {
    ...
    */
    ...
    fn backend_options_eq(a: &wgpu_29::BackendOptions, b: &wgpu_29::BackendOptions) -> bool {
        let wgpu_29::BackendOptions { gl, dx12, noop } = a;
        gl_backend_options_eq(gl, &b.gl)
            && dx12_backend_options_eq(dx12, &b.dx12)
            && noop_backend_options_eq(noop, &b.noop)
    }

    fn gl_backend_options_eq(a: &wgpu_29::GlBackendOptions, b: &wgpu_29::GlBackendOptions) -> bool {
        let wgpu_29::GlBackendOptions { gles_minor_version, fence_behavior, debug_fns } = a;
    ...
    fn dx12_backend_options_eq(
        a: &wgpu_29::Dx12BackendOptions,
        b: &wgpu_29::Dx12BackendOptions,
    ) -> bool {
        let wgpu_29::Dx12BackendOptions {
            shader_compiler,
            presentation_system,
    ...
    fn dx12_compiler_eq(a: &wgpu_29::Dx12Compiler, b: &wgpu_29::Dx12Compiler) -> bool {
        use wgpu_29::Dx12Compiler as C;
        match (a, b) {
    ...
    fn dxc_shader_model_opt_eq(
        a: &Option<wgpu_29::DxcShaderModel>,
        b: &Option<wgpu_29::DxcShaderModel>,
    ) -> bool {
        match (a, b) {
            (None, None) => true,
            // Fieldless enum without `PartialEq`; the discriminant is the whole value.
    ...
    fn noop_backend_options_eq(
        a: &wgpu_29::NoopBackendOptions,
        b: &wgpu_29::NoopBackendOptions,
    ) -> bool {
        let wgpu_29::NoopBackendOptions { enable } = a;
        *enable == b.enable
    }

    fn memory_budget_thresholds_eq(
        a: &wgpu_29::MemoryBudgetThresholds,
        b: &wgpu_29::MemoryBudgetThresholds,
    ) -> bool {
        let wgpu_29::MemoryBudgetThresholds { for_resource_creation, for_device_loss } = a;
        *for_resource_creation == b.for_resource_creation && *for_device_loss == b.for_device_loss
    }

    fn memory_hints_eq(a: &wgpu_29::MemoryHints, b: &wgpu_29::MemoryHints) -> bool {
        use wgpu_29::MemoryHints as M;
        match (a, b) {
