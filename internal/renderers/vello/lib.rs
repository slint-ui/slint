    ...
    fn internal_render_with_post_callback(
        ...
        */
        ...
        let base_color = window_inner
            ...
            .unwrap_or(vello::peniko::color::palette::css::WHITE);
...
impl RendererSealed for VelloRenderer {
    ...
    fn resize(&self, size: i_slint_core::api::PhysicalSize) -> Result<(), PlatformError> {
        ...
        if let (Some(device), Some(config)) = (device, config.as_mut()) {
            config.width = width.get();
            config.height = height.get();
            if let Some(surface) = self.surface.borrow().as_ref() {
                surface.configure(&device, config);
            }
            *self.surface_config.borrow_mut() = Some(config.clone());

            *self.target_texture.borrow_mut() =
