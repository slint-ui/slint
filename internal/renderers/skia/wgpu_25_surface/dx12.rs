...
use windows::Win32::Graphics::Direct3D12::{
    D3D12CreateDevice, ID3D12Device, ID3D12Resource, D3D12_RESOURCE_STATE_PRESENT,
};
use windows::Win32::Graphics::Dxgi::Common::DXGI_STANDARD_MULTISAMPLE_QUALITY_PATTERN;
use windows::Win32::Graphics::Dxgi::{
    Common::{DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_FORMAT_R8G8B8A8_UNORM_SRGB},
...
fn find_matching_adapter(device: &ID3D12Device) -> Option<skia_safe::gpu::d3d::IDXGIAdapter1> {
    unsafe {
        let dxgi_factory: IDXGIFactory4 = CreateDXGIFactory2(DXGI_CREATE_FACTORY_FLAGS(0)).unwrap();

        let mut index = 0;
        loop {
            let adapter_result = dxgi_factory.EnumAdapters1(index);
            if let Err(_) = adapter_result {
                break;
            }
