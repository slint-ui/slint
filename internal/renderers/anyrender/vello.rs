...
impl VelloWindowRenderer {
    ...
    fn create_state(
        ...
        surface_config.format = capabilities
            ...
            .or_else(|| {
                capabilities.formats.iter().find(|format| {
        ...
        if self.transparent {
            ...
            if let Some(mode) =
                [PreMultiplied, PostMultiplied].into_iter().find(|m| capabilities.alpha_modes.contains(m))
            {
...
impl SlintWindowRenderer for VelloWindowRenderer {
        ...
        F: FnOnce(&mut Self::ScenePainter<'_>) -> Result<(), PlatformError>,
    {
        ...
        let Some(ActiveState {
            device, queue, surface, surface_config, target, blitter, renderer, ..
        }) = state
        else {
            return Ok(());
        };
        ...
        let surface_texture = match surface.get_current_texture() {
            ...
            // Skip the frame; the surface is reconfigured on the next resize,
            // resume, or when the window becomes visible again.
            _ => return Ok(()),
        };
