...
struct SubComponentInstance {
    ...
    //callbacks: Pin<Box<[Callback<[Value], Value>]>>,
    properties: TiVec<llr::PropertyIdx, Pin<Box<Property<Value>>>>,
    callbacks: TiVec<llr::CallbackIdx, Pin<Box<Callback<[Value], Value>>>>,
    change_trackers: Vec<Pin<Box<ChangeTracker>>>,
...
impl SubComponentInstance {
    fn instantiate(
        ...
        parent_ctx: Option<&llr::ParentScope>,
    ) -> Self {
        ...
        let mut properties = TiVec::default();
        properties.resize_with(component.properties.len(), || Box::pin(Property::default()));
        let mut callbacks = TiVec::default();
        callbacks.resize_with(component.callbacks.len(), || Box::pin(Callback::default()));

        let mut change_trackers = Vec::default();
        change_trackers
            .resize_with(component.change_callbacks.len(), || Box::pin(ChangeTracker::default()));
    ...
    fn init(&self, component_idx: llr::SubComponentIdx, item_tree_rc: &DynamicItemTreeRc) {
        ...
        for (idx, sci) in component.sub_components.iter_enumerated() {
            self.sub_components[idx].init(sci.ty, item_tree_rc);
        }

        for (prop1, prop2, fields) in &component.two_way_bindings {
            let prop1 = self.get_property_or_callback(prop1).unwrap().left().unwrap();
            let prop2 = self.get_property_or_callback(prop2).unwrap().left().unwrap();
            let common = prop2.prepare_for_two_way_binding();
            let map = (!fields.is_empty()).then(|| {
                ...
                impl rtti::TwoWayBindingMapping<Value> for FieldAccess {
                    fn map_to(&self, value: &Value) -> Value {
                        ...
                        for f in &self.0 {
                            match value {
                                Value::Struct(o) => {
                                    value = o.get_field(f).cloned().unwrap_or_default()
                                }
                                Value::Void => return Value::Void,
                                _ => {
                                    panic!("Cannot map to a field of a non-struct {value:?}  - {f}")
                                }
                            }
                        }
                        value
                ...
                Rc::new(FieldAccess(fields.clone())) as Rc<dyn rtti::TwoWayBindingMapping<Value>>
            });
            prop1.link_two_way_with_map(common, map);
        }
            ...
            match self.get_property_or_callback(prop).unwrap() {
                itertools::Either::Left(prop) => {
                    if init.is_constant {
                        ...
                        prop.set_binding(
                            Box::new(move || {
                                let Some(item_tree_rc) = item_tree_weak.upgrade() else {
                                    return Value::Void;
                                };
                                let component = &item_tree_rc.llr.sub_components[component_idx];
                ...
                itertools::Either::Right(cb) => {
                    let item_tree_weak = VRc::downgrade(&item_tree_rc);
                    cb.set_handler(Box::new(move |args: &[Value]| -> Value {
                        let Some(item_tree_rc) = item_tree_weak.upgrade() else {
                            return Value::Void;
                        };
                        let component = &item_tree_rc.llr.sub_components[component_idx];
    ...
    fn get_property_or_callback<'a>(
        ...
        match &reference.reference {
            ...
            llr::LocalMemberIndex::Function(..) => unreachable!(),
            llr::LocalMemberIndex::Callback(callback_idx) => {
                PropertyOrCallback::Right(s.callbacks[*callback_idx].as_ref()).into()
            }
            llr::LocalMemberIndex::Native { item_index, prop_name } => {
                s.items[*item_index].as_ref().get_property_or_callback(prop_name)
            }
        }
    }
}

type DynamicItemTreeRc = VRc<ItemTreeVTable, DynamicItemTree>;

struct DynamicItemTree {
    llr: Rc<llr::CompilationUnit>,
    root_component: SubComponentInstance,
    item_array_cache: Rc<ItemTreeArrayCache>,
    self_weak: vtable::VWeak<ItemTreeVTable, DynamicItemTree>,
}
...
impl DynamicItemTree {
    ...
    fn item_tree(&self) -> &llr::ItemTree {
        todo!()
    }
}

struct ItemTreeArrayCache {
    item_tree_array: Vec<ItemTreeNode>,
...
impl i_slint_core::item_tree::ItemTree for DynamicItemTree {
    fn visit_children_item(
        ...
        i_slint_core::item_tree::visit_item_tree(
            self,
            &vtable::VRc::into_dyn(self_rc.clone()),
            &self.item_array_cache.item_tree_array,
            index,
            order,
            visitor,
            |_, order, visitor, index| todo!(),
        )
    }
    ...
    fn get_subtree(
        ...
        _1: u32,
        _2: usize,
        _3: &mut vtable::VWeak<ItemTreeVTable, Dyn>,
    ) {
        todo!()
    }
    ...
    fn parent_node(self: core::pin::Pin<&Self>, _1: &mut ItemWeak) {
        todo!()
    }
    ...
    fn item_geometry(self: core::pin::Pin<&Self>, _1: u32) -> LogicalRect {
        todo!()
    }
    fn accessible_role(self: core::pin::Pin<&Self>, _1: u32) -> AccessibleRole {
        todo!()
    }
    ...
    fn item_element_infos(self: core::pin::Pin<&Self>, _1: u32, _2: &mut SharedString) -> bool {
        todo!()
    }

    fn window_adapter(self: core::pin::Pin<&Self>, _1: bool, _2: &mut Option<WindowAdapterRc>) {
        todo!()
    }
}
