    ...
    let update = {
        ...
        move || {
            ...
            for (idx, t) in sc.timers.iter().enumerate() {
                ...
                let Some(timer) = owner.timers.get(idx) else { continue };
...
fn install_change_callbacks(
    ...
    let base = 2 * sc.timers.len();
    for (idx, (target, expr)) in sc.change_callbacks.iter().enumerate() {
        let target = target.clone();
        let expr = mut_expression_clone(expr);
        let weak_get = weak_sub.clone();
        let weak_set = weak_sub.clone();
        let expr_for_set = expr.clone();
        sub.change_trackers[base + idx].init(
            (),
            move |()| -> Value {
                let Some(owner) = weak_get.upgrade() else { return Value::Void };
...
fn install_global_property_init(
    ...
    let g = &sub.compilation_unit.globals[global_index];
    match member {
        LocalMemberIndex::Property(idx) => {
        ...
        LocalMemberIndex::Callback(idx) => {
            ...
            if let Some(native) = &global.native {
                let _ = native
                    .as_ref()
                    .set_callback_handler(&g.callbacks[*idx].name, Box::new(handler));
            } else {
                Pin::as_ref(&global.callbacks[*idx]).set_handler(handler);
            }
        }
        LocalMemberIndex::Function(_)
...
/// `int` (i32) and `duration` (i64) properties interpolate with per-frame
/// rounding; reproduce that when the animation runs on a type-erased
/// `Property<Value>`.
...
fn run_init_code(sub: &Pin<Rc<SubComponentInstance>>) {
    // Nested sub-components run their init code first, so the outer
    // component's use-site overrides (in its own init_code) observe the
    // inner component's initialization.
    for nested in &sub.sub_components {
