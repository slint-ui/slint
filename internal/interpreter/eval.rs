...
fn call_builtin_function(
    ...
    match f {
        ...
        BuiltinFunction::StringIsAscii => {
            if arguments.len() != 1 {
                panic!("internal error: incorrect argument count to StringIsAscii")
            }
            if let Value::String(s) = eval_expression(&arguments[0], local_context) {
        ...
        BuiltinFunction::StringRepeat => {
            if arguments.len() != 2 {
                panic!("internal error: incorrect argument count to StringRepeat")
            }
            if let Value::String(s) = eval_expression(&arguments[0], local_context) {
                if let Value::Number(n) = eval_expression(&arguments[1], local_context)
                    && n >= 0.0
                    && n == n.trunc()
                {
                    Value::String(s.repeat(n as usize).into())
                } else {
                    panic!("Second argument not a non-negative integer");
                }
            } else {
                panic!("First argument not a string");
            }
        ...
        BuiltinFunction::StringTrim => {
            if arguments.len() != 1 {
                panic!("internal error: incorrect argument count to StringTrim")
            }
            if let Value::String(s) = eval_expression(&arguments[0], local_context) {
