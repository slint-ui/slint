...
impl RepeaterOrConditional {
    ...
    /// virtualized row layout. The interpreter's content properties may
    /// live on a native item (e.g. `Flickable::content-y`), which
...
pub struct SubComponentInstance {
    ...
    /// Change trackers for the timers (two per timer, first) and the
    /// `change_callbacks` (in declaration order, after).
    pub change_trackers: Vec<ChangeTracker>,
    ...
    pub timers: Vec<i_slint_core::timers::Timer>,
    ...
    /// popup to tear down.
    pub popup_ids: Vec<std::cell::Cell<Option<std::num::NonZeroU32>>>,
...
impl Drop for Instance {
    fn drop(&mut self) {
        // Free the per-component renderer caches (text shaping, bounding rects, …)
...
impl Instance {
    ...
    pub fn ensure_updated(&self, tree_index: u32) -> bool {
        ...
        if let Some(lv) = listview_info.as_ref() {
            ...
            let props = ValueListViewProps {
                content_y: lv.content_y.clone(),
                content_width: lv.content_width.clone(),
                content_height: lv.content_height.clone(),
                ctx_sub: sub.clone(),
            };
    ...
    pub fn visit_dynamic_children(
        ...
        // the model or the ListView content geometry changes.
        let repeater = &sub.repeaters[rep_idx];
        let sc = &sub.compilation_unit.sub_components[sub.sub_component_idx];
    ...
    pub fn new_with_window(
        ...
        Self::new_with_options(compilation_unit, public_component_index, window_adapter, None)
    }
    ...
    pub fn new_repeated(
        ...
        let vrc = build_instance(&compilation_unit, item_tree, parent.clone(), globals, None);
    ...
    pub fn new_popup(
        ...
        build_instance(&compilation_unit, item_tree, parent, globals, None)
    }
}
...
fn build_sub_component_instance(
    ...
    let rc = Rc::new_cyclic(|weak_self: &Weak<SubComponentInstance>| {
        ...
        SubComponentInstance {
            ...
            parent,
            root: OnceCell::new(),
            change_trackers: std::iter::repeat_with(ChangeTracker::default)
                .take(2 * sc.timers.len() + sc.change_callbacks.len())
                .collect(),
            timers: std::iter::repeat_with(Default::default).take(sc.timers.len()).collect(),
            popup_ids: vec![std::cell::Cell::new(None); sc.popup_windows.len()],
...
/// the interpreter's `Value`-typed content storage. The content
/// references may be user-declared `Property<Value>` fields *or* native
/// item properties (e.g. `Flickable::content-y`); routing through
/// `load_property` / `store_property` handles both uniformly.
struct ValueListViewProps {
    content_y: llr::MemberReference,
    /// `None` when the user set `content-width` explicitly, in which case
    /// the ListView must not overwrite it (see #12264).
    content_width: Option<llr::MemberReference>,
    content_height: Option<llr::MemberReference>,
    ctx_sub: Pin<Rc<SubComponentInstance>>,
}

impl i_slint_core::model::ListViewProperties for ValueListViewProps {
    fn content_y_get(&self) -> i_slint_core::lengths::LogicalLength {
        read_logical_length(&self.ctx_sub, &self.content_y)
    }
    fn content_y_get_internal(&self) -> i_slint_core::lengths::LogicalLength {
        // The rtti route has no equivalent of `Property::get_internal`;
        // reading normally only differs while a physics animation drives
        // `content-y`, where it may re-evaluate the animated binding.
        read_logical_length(&self.ctx_sub, &self.content_y)
    }
    fn content_y_set(&self, value: i_slint_core::lengths::LogicalLength) {
    ...
    fn content_y_has_binding(&self) -> bool {
        // Unlike the generated code, the interpreter doesn't track whether
        // the underlying property has an external binding; `false` lets
    ...
    fn content_width_set(&self, value: i_slint_core::lengths::LogicalLength) {
        let Some(content_width) = &self.content_width else { return };
        let ctx = crate::eval::EvalContext::new(self.ctx_sub.clone());
        crate::eval::store_property(&ctx, content_width, crate::Value::Number(value.get() as f64));
    }
    fn content_height_set(&self, value: i_slint_core::lengths::LogicalLength) {
        let Some(content_height) = &self.content_height else { return };
        let ctx = crate::eval::EvalContext::new(self.ctx_sub.clone());
        crate::eval::store_property(&ctx, content_height, crate::Value::Number(value.get() as f64));
    }
    fn register_as_dependencies(&self) {
        ...
        if let Some(content_width) = &self.content_width {
            let _ = read_logical_length(&self.ctx_sub, content_width);
        }
        if let Some(content_height) = &self.content_height {
            let _ = read_logical_length(&self.ctx_sub, content_height);
        }
        let _ = read_logical_length(&self.ctx_sub, &self.content_y);
    }
}
...
/// the repeated instance's root sub-component.
impl i_slint_core::model::RepeatedItemTree for Instance {
    ...
    fn listview_layout(
        ...
        // Write `prop_y` on the repeated row's root sub-component, advance
        // `offset_y` by `prop_height`, and return the row's preferred
        // horizontal layout info width as the new content width estimate.
        let this = self.get_ref();
    ...
    fn flexbox_layout_item_info(
        ...
        if let Some(expr) = &sc.flexbox_layout_item_info_for_repeated {
            ...
            // Mirror for the other axis: a width-for-height instance (e.g. a
            // wrapping column FlexboxLayout) must not read self.height. Use the
            // constrained horizontal info (computed at an unbounded height).
            if matches!(orientation, i_slint_core::items::Orientation::Horizontal)
                && child_index.is_none()
                && let Some(h_expr) = &sc.layout_info_h_constrained_for_repeated
            {
                let mut ctx = crate::eval::EvalContext::new(self.root_sub_component.clone());
...
impl Instance {
    ...
    /// Horizontal flexbox info for a repeated instance measured at the assigned
    /// cross height, so a width-for-height cell resolves to the same width as
    ...
    pub fn flexbox_layout_item_info_at_cross_height(
        ...
        use i_slint_core::model::RepeatedItemTree;
        let mut info =
            RepeatedItemTree::flexbox_layout_item_info(self, Orientation::Horizontal, None);
        let cu = self.root_sub_component.compilation_unit.clone();
        let sc = &cu.sub_components[self.root_sub_component.sub_component_idx];
