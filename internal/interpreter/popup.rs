...
pub(crate) fn show_popup_window(ctx: &mut EvalContext, arguments: &[Expression]) -> Value {
    ...
    if let Some(slot) = owner.popup_ids.get(*popup_index as usize) {
        slot.set(Some(popup_id));
    }
...
pub(crate) fn show_popup_menu(ctx: &mut EvalContext, arguments: &[Expression]) -> Value {
    ...
    // Store the popup id on the native `ContextMenu` item, so its
    // `is-open()` and `close()` member functions see this popup as the
    // active one.
