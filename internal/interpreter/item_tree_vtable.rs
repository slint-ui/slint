...
impl i_slint_core::item_tree::ItemTree for Instance {
    ...
    fn item_geometry(self: Pin<&Self>, item_index: u32) -> LogicalRect {
        ...
        // binding. So read the wrapper's geometry from the *parent*
        // sub-component at the placement slot and never query the inner
        // root. Fall through when the parent
    ...
    fn item_element_infos(self: Pin<&Self>, item_index: u32, result: &mut SharedString) -> bool {
        ...
        // into each sub-component's local tree space.
        //
        ...
        // Check each level before descending — first match wins.
        ...
        for &sub_step in entry.0.iter() {
            ...
            // Translate `local_idx` into `nested`'s tree.
...
/// deepest item itself, so an outer-element query wins over the inner
/// sub-component root's own accessible properties.
