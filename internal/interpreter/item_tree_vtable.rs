...
impl i_slint_core::item_tree::ItemTree for Instance {
    ...
    fn ensure_instantiated(self: Pin<&Self>) -> bool {
        self.get_ref().ensure_instantiated()
    }
    ...
    fn parent_node(self: Pin<&Self>, result: &mut ItemWeak) {
        ...
        let this = self.get_ref();
        // `embedded_in` records where in the outer item tree this instance
        // lives. Return that as the parent; the core walks back through it
        // the same way as a repeated DynamicTree node.
    ...
    fn item_geometry(self: Pin<&Self>, item_index: u32) -> LogicalRect {
        ...
        // and lands on its root element (local tree index 0), the inner
        // root's geometry can duplicate the parent's placement: the
        // compiler's `adjust_geometry_for_injected_parent` pass hoists the
        // original position into an injected wrapper item, and the inner
        // root applies the same offset again via its `y: root-1_y`
        // binding. The rust codegen reads the wrapper's geometry from the
        // *parent* sub-component at the placement slot and never queries
        // the inner root; do the same here. Fall through when the parent
        // has no entry (the sub-component was placed directly with no
        // wrapper, e.g. `box := SpinBox {}` inside a Window — then the
        // inner root's own geometry is the correct placement).
    ...
    fn supported_accessibility_actions(
        ...
        for (owner, local_idx) in resolve_accessible_candidates(self.get_ref(), item_index) {
            let cu = owner.compilation_unit.clone();
            let sc = &cu.sub_components[owner.sub_component_idx];
            for (idx, key) in sc.accessible_prop.keys() {
                if *idx == local_idx
                    && let Some(action_name) = key.strip_prefix("Action")
                {
                    actions |= SupportedAccessibilityAction::from_name(action_name)
                        .unwrap_or_else(|| panic!("Not an accessible action: {action_name:?}"));
                }
            }
        }
        actions
    ...
    fn item_element_infos(self: Pin<&Self>, item_index: u32, result: &mut SharedString) -> bool {
        ...
        // into each sub-component's local tree space like the rust
        // codegen's nested `item_element_infos` match arms
        // (`index - range_begin + 1` at every recursion).
        //
        // A native item's info lives on the leaf sub-component; a
        // component-instance declaration's (`Switch { }`) lives on the
        // *parent* of the leaf, keyed by the instance's `index_in_tree`.
        // Check each level before descending — first match wins, like the
        // codegen's direct-entry cases that precede the recursion.
        ...
        for &sub_step in entry.0.iter() {
            ...
            // Translate `local_idx` into `nested`'s tree, like the rust
            // codegen's `index - range_begin + 1` recursion (and the
            // direct case for the root item).
...
/// The `accessible_prop` map key for a string property — the same
/// PascalCase form the lowering derives from the enum's kebab-case
/// `Display` (see `lower_to_item_tree`).
fn accessible_string_property_name(what: AccessibleStringProperty) -> String {
    i_slint_compiler::generator::to_pascal_case(&what.to_string())
}
