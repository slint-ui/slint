...
#[cfg(feature = "markdown")]
#[test]
fn paragraph_block_basics() {
    let text = ParagraphBlock::Text(RichText {
        text: "hello".into(),
        formatting: alloc::vec::Vec::new(),
        links: alloc::vec::Vec::new(),
    });
    assert_eq!(rich_text_content(&text).map(|rt| rt.text.as_str()), Some("hello"));
    let heading = ParagraphBlock::Heading { level: 1, content: RichText::default() };
    assert_eq!(rich_text_content(&heading).map(|rt| rt.text.as_str()), Some(""));
    assert_eq!(rich_text_content(&ParagraphBlock::HorizontalRule), None);
}
...
fn markdown_heading_levels() {
    ...
    assert_eq!(
        assert_no_errors(parse_interpolated::<&[_]>("### Sub", &[])),
        [ParagraphBlock::Heading {
            level: 3,
            content: RichText {
                text: "Sub".into(),
    ...
    // Unclosed heading at end of input
    assert_eq!(
        assert_no_errors(parse_interpolated::<&[_]>("# Title", &[])),
        [ParagraphBlock::Heading {
            level: 1,
...
fn markdown_horizontal_rule() {
    assert_eq!(
        assert_no_errors(parse_interpolated::<&[_]>("above\n\n---\n\nbelow", &[])),
        [
            ParagraphBlock::Text(RichText {
                text: "above".into(),
                formatting: alloc::vec![],
