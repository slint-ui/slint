...
pub(crate) struct TouchFingerIdAllocator {
    /// Index in this vector = the allocated finger id; entry = the winit touch
...
impl TouchFingerIdAllocator {
    ...
    pub(crate) fn id_for(&mut self, winit_id: u64) -> i32 {
        let index = self
            .slot_of(winit_id)
...
mod tests {
    ...
    fn allocates_and_frees_slots() {
        ...
        assert_eq!(alloc.id_for(11), 0);
        assert_eq!(alloc.id_for(22), 1);
    ...
    fn handles_ids_outside_i32_range() {
        ...
        assert_eq!(alloc.id_for(sign_extended), 0);
    ...
    fn tracks_more_fingers_than_one_pair_of_hands() {
        let mut alloc = TouchFingerIdAllocator::default();
        for i in 0..20 {
            assert_eq!(alloc.id_for(100 + i as u64), i);
        }
        for i in 0..20 {
            assert_eq!(alloc.take(100 + i as u64), Some(i));
        }
    }
}
