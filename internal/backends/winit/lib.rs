...
fn create_renderer(
    ...
    match (shared_data.renderer_name.as_deref(), shared_data.requested_graphics_api.as_ref()) {
        ...
        #[cfg(all(enable_skia_renderer, feature = "unstable-wgpu-29"))]
        (Some("skia-wgpu"), maybe_graphics_api) => {
            if let Some(selected_renderer) = maybe_graphics_api.map_or_else(
                || Some(renderer::skia::WinitSkiaRenderer::new_wgpu_29_suspended(shared_data)),
                |api| {
                    if matches!(api, RequestedGraphicsAPI::WGPU29(..)) {
