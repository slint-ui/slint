...
impl EventLoopState {
    ...
    fn start_pending_drag(&mut self, event_loop: &dyn ActiveEventLoop) {
    ...
    fn dispatch_incoming_drag(
        ...
        // through.
        drop_event.allow_copy = true;
        drop_event.allow_move = true;
        drop_event.allow_link = true;
...
/// Map a winit drag action to Slint's `DragAction`. A `None` (unknown) action becomes
/// `DragAction::None`.
...
/// did not supply one (some platforms only report the action when the drop completes).
...
impl winit::application::ApplicationHandler for EventLoopState {
    ...
    fn window_event(
        ...
        // `ActiveEventLoop` is in hand.
        self.start_pending_drag(event_loop);
