...
use sixtyfps_corelib::input::MouseEvent;
...
impl Devices for TerminalDevices {
    ...
    fn fill_region(&mut self, region: sixtyfps_corelib::graphics::IntRect, pixels: &[Rgb888]) {
        ...
        for y in (0..region.height()).step_by(2) {
            for x in 0..w {
                ...
                let p2 = pixels
                    ...
                    .or_else(|| {
                        self.frame_buffer.get(a_x as usize + (1 + 2 * a_y as usize) * a_size.width)
                    })
                    .copied()
                    .map_or(crossterm::style::Color::Reset, map_color);
                stdout
        ...
        /*for (it, pix) in pixels.iter().copied().enumerate() {
            let x = (region.min_x() + it as i32 % w) as u16;
            let y = (region.min_y() + it as i32 / w) as u16;
            stdout
                .queue(MoveTo(x, y))
                .unwrap()
