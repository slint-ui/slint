...
/// Overwrites `//` and `/* */` comments with spaces, so that a plain JSON parser
/// accepts the file. Newlines and the length are kept, so the line and column of a
/// parse error still point at the file on disk.
///
/// Scanning bytes is enough: every byte of a multi-byte character has its high bit
/// set, so none of them can be mistaken for a quote or a slash.
