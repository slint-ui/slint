...
impl PyComponentsDifference {
    fn compare(orig: &[PyComponent], new: &[PyComponent]) -> Option<Self> {
        ...
        let added_components = new_components
            .iter()
            .filter_map(|(name, _)| {
                if orig_components.contains_key(name) {
                    None
                } else {
                    Some((*name).into())
                }
    ...
    fn compare(old_compo: &PyComponent, new_compo: &PyComponent) -> Option<Self> {
        ...
        let added_properties = new_props
            ...
            .filter_map(|(name, new_prop)| {
                if orig_props.contains_key(name) {
                    None
                } else {
                    Some((*new_prop).clone())
                }
    ...
    fn compare(orig: &[PyStructOrEnum], new: &[PyStructOrEnum]) -> Option<Self> {
        ...
        let added_structs =
            new_structs
                .iter()
                .filter_map(|(name, _)| {
                    if orig_structs.contains_key(name) {
                        None
                    } else {
                        Some((*name).into())
                    }
        ...
        let added_enums = new_enums
            ...
            .filter_map(
                |(name, _)| {
                    if orig_enums.contains_key(name) {
                        None
                    } else {
                        Some((*name).into())
                    }
        ...
        let removed_structs =
            orig_structs
                .iter()
                .filter_map(|(name, _)| {
                    if new_structs.contains_key(name) {
                        None
                    } else {
                        Some((*name).into())
                    }
    ...
    fn compare(old_struct: &PyStruct, new_struct: &PyStruct) -> Option<Self> {
        ...
        let added_fields = new_fields
            ...
            .filter_map(|(name, new_field)| {
                if orig_fields.contains_key(name) {
                    None
                } else {
