...
impl TsComponent {
    fn generate(&self, ty: ComponentType<'_>, file: &mut typescript_ast::File) {
        let mut iface = typescript_ast::Interface {
        ...
        file.declarations.push(typescript_ast::Declaration::Interface(iface));
...
mod typescript_ast {
    ...
    impl Display for Declaration {
        fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
            match self {
                Declaration::Interface(iface) => write!(f, "{}", iface),
...
fn ts_type_name(ty: &Type) -> SmolStr {
    match ty {
        ...
        Type::Struct(s) => match &s.name {
            ...
            StructName::BuiltinPrivate(_) => SmolStr::new_static("void"),
