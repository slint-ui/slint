...
impl PyGlobal {
    fn generate(&self, file: &mut File) {
        let mut class = Class { name: self.name.clone(), ..Default::default() };

        class.fields = self.properties.iter().map(From::from).collect();
...
impl From<&llr::GlobalComponent> for PyGlobal {
...
impl PyComponent {
    fn generate<'a>(&self, globals: impl Iterator<Item = &'a PyGlobal>, file: &mut File) {
        let mut class = Class {
...
pub fn generate_py_module(
    ...
    module.globals.extend(globals.clone().map(PyGlobal::from));
...
pub fn generate(
    ...
    for global in &pymodule.globals {
        global.generate(&mut file);
    }

    for public_component in &pymodule.components {
        public_component.generate(pymodule.globals.iter(), &mut file);
    }
