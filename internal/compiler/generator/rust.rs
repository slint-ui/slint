        ...
        Expression::TranslationReference { format_args, string_index, plural } => {
            ...
            match plural {
                Some(plural) => {
                    ...
                    quote!(sp::translate_bundle_with_plural(
                ...
                None => quote!(sp::translate_bundle(&self::_SLINT_TRANSLATED_STRINGS[#string_index], sp::Slice::<sp::SharedString>::from(#args).as_slice())),
    ...
    let rules = translations.plural_rules.iter().map(|rule| {
        let rule = match rule {
            Some(rule) => {
                ...
                quote!(Some(|arg_0 : i32| #rule as usize))
            }
            None => quote!(None),
        };
        quote!(#rule)
