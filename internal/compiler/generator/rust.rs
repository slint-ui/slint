...
use crate::llr::{
    ...
    MouseCursor, ParentScope, TypeResolutionContext as _,
};
...
fn compile_expression(expr: &Expression, ctx: &EvaluationContext) -> TokenStream {
    match expr {
        ...
        Expression::MouseCursor(MouseCursor::CustomCursor { image, hotspot_x, hotspot_y }) => {
            let img = compile_expression(image.as_ref(), ctx);
            let hotspot_x = compile_expression(hotspot_x.as_ref(), ctx);
            let hotspot_y = compile_expression(hotspot_y.as_ref(), ctx);
            quote!(sp::MouseCursor::CustomCursor { image: #img, hotspot_x: #hotspot_x as i32, hotspot_y: #hotspot_y as i32 })
        }
