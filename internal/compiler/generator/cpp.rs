...
impl CppType for Type {
    fn cpp_type(&self) -> Option<SmolStr> {
        match self {
            ...
            Type::MouseCursor => Some("slint::cbindgen_private::MouseCursor".into()),
            _ => None,
        }
    }
}
...
fn compile_expression(expr: &llr::Expression, ctx: &EvaluationContext) -> String {
    ...
    match expr {
        ...
        Expression::MouseCursor(cursor) => match cursor {
            llr::MouseCursorInner::BuiltIn(cursor) => compile_expression(cursor.as_ref(), ctx),
            llr::MouseCursorInner::CustomMouseCursor { image, hotspot_x, hotspot_y } => {
                ...
                format!(
                    "slint::cbindgen_private::MouseCursor(slint::cbindgen_private::MouseCursor::Tag::CustomMouseCursor, {image}, {hotspot_x}, {hotspot_y})"
                )
            }
        },
