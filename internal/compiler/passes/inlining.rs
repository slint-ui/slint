    ...
    fn inline_components_recursively(
        ...
        recurse_elem_no_borrow(&component.root_element, &(), &mut |elem, _| {
            ...
            if let ElementType::Component(c) = base {
                ...
                if c.parent_element.borrow().upgrade().is_some() {
                ...
                if match inline_selection {
                    ...
                    InlineSelection::InlineOnlyRequiredComponents => {
                        component_requires_inlining(&c)
                            ...
                            || component.parent_element.borrow().upgrade().is_none() && Rc::ptr_eq(elem, &component.root_element)
...
fn inline_element(
    ...
    debug_assert!(inlined_component.parent_element.borrow().upgrade().is_none());
    ...
    if let ElementType::Component(c) = &mut elem_mut.base_type
        && c.parent_element.borrow().upgrade().is_some()
    {
        debug_assert!(Rc::ptr_eq(elem, &c.parent_element.borrow().upgrade().unwrap()));
...
fn duplicate_element_with_mapping(
    ...
    if let ElementType::Component(c) = &mut new.borrow_mut().base_type
        && c.parent_element.borrow().upgrade().is_some()
    {
        debug_assert!(Rc::ptr_eq(element, &c.parent_element.borrow().upgrade().unwrap()));
...
fn duplicate_sub_component(
    ...
    debug_assert!(component_to_duplicate.parent_element.borrow().upgrade().is_some());
    ...
    *new_component.menu_item_tree.borrow_mut() = component_to_duplicate
        ...
        .map(|it| {
            let new_parent =
                mapping.get(&element_key(it.parent_element.borrow().upgrade().unwrap())).unwrap().clone();
...
fn duplicate_popup(p: &PopupWindow, mapping: &mut Mapping, priority_delta: i32) -> PopupWindow {
    let parent = mapping
        .get(&element_key(p.component.parent_element.borrow().upgrade().expect("must have a parent")))
