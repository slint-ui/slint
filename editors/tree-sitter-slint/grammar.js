...
module.exports = grammar({
    ...
    rules: {
        ...
        export_statement: ($) =>
            seq("export", "{", commaSep($._type_identifier), "}"),
        ...
        struct_field_declaration_list: ($) =>
            seq($.struct_field_declaration_list_body, "}"),
        ...
        field_declaration_list: ($) => seq($.field_declaration_list_body, "}"),
        ...
        transitions_definition: ($) =>
            seq("transitions", $.transitions_list_definition),
        ...
        states_definition: ($) => seq("states", $.states_list_definition),
        ...
        animate_statement: ($) =>
            seq("animate", $.var_identifier, $.animate_declaration_list),
        animate_declaration_list: ($) =>
            seq(
                "{",
                repeat(seq($.builtin_type_identifier, ":", $._expression, ";")),
                "}"
            ),

        callback_event: ($) =>
            seq($.function_identifier, "=>", $.field_declaration_list),
        callback_call: ($) => seq($.var_identifier, ";"),
        ...
        if_statement_definition: ($) =>
            seq(
                ...
                choice(
                    seq($.field_declaration_list),
                    seq(":", $.component_definition)
                )
            ),
        ...
        property_expr: ($) =>
            seq(choice("=", ":"), choice($._expression, $.list_definition)),

        _propterty_kind: ($) =>
            choice(field("type", $._type_identifier), $.property_type_list),

        property_type_list: ($) =>
            seq("[", field("type", $._type_identifier), "]"),

        list_definition: ($) => seq($.list_definition_body, "]"),

        list_definition_body: ($) =>
            seq(
                "[",
                repeat(seq($.struct_field_declaration_list, optional(",")))
            ),

        variable_definition: ($) =>
            seq(field("name", $.var_identifier), ":", $._expression, ";"),
        ...
        _expression: ($) => choice($._expression_body, $.expression_body_paren),

        expression_body_paren: ($) => seq("(", $._expression_body, ")"),
        ...
        formal_parameters: ($) =>
            seq(
                "(",
                optional(seq(commaSep1($.formal_parameter), optional(","))),
                ")"
            ),
        ...
        unary_prec_operator: ($) => choice("!", "-"),

        add_prec_operator: ($) => choice("+", "-"),
        mult_prec_operator: ($) => choice("*", "/", "&&", "||"),
        ...
        _type_identifier: ($) =>
            choice($.user_type_identifier, $.builtin_type_identifier),
        var_identifier: ($) =>
            seq(
                choice(
                    $._identifier,
        ...
        reference_identifier: ($) => choice("parent", "root", "this"),

        value: ($) =>
            choice($.value_with_units, $.number, $.language_constant, $.color),

        color: ($) => seq("#", choice(/[\da-zA-Z]{3}/, /[\da-zA-Z]{6}/)),

        value_with_units: ($) => seq($.number, $.unit_type),
        number: ($) => choice($.int_number, $.float_number),
        int_number: ($) => /\d+/,
        float_number: ($) => /\d+\.\d+/,

        unit_type: ($) => choice("px", "%", "ms"),
