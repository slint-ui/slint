...
struct AdapterInner {
    ...
    readable_callback: Option<PyObject>,
    writable_callback: Option<PyObject>,
}
    ...
    fn restart_after_mut_inner_access(&mut self, callback: impl FnOnce(&mut AdapterInner)) {
        ...
        self.task = Some(
            slint_interpreter::spawn_local(std::future::poll_fn(move |cx| loop {
                ...
                let readable_poll_status: Option<std::task::Poll<PyObject>> =
                    inner.readable_callback.as_ref().map(|callback| {
                        if inner.adapter.poll_readable(cx).is_ready() {
                            std::task::Poll::Ready(Python::with_gil(|py| callback.clone_ref(py)))
                        } else {
                            std::task::Poll::Pending
                        }
                    });

                let writable_poll_status: Option<std::task::Poll<PyObject>> =
