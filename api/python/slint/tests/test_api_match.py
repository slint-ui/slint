...
def test_version_mismatch() -> None:
    """A wrapper generated by an older Slint must be rejected with a clear
    'regenerate' message, regardless of whether the rest of the descriptor
    still matches."""
    with pytest.raises(RuntimeError) as excinfo:
        _load_file_checked(
            base_dir() / "api-match.slint",
    ...
    assert "no longer compatible" in message
