...
/// States where the generated code around it comes from in the `.slint`
/// source, as text: what the code is (an element's painting, a binding's
/// evaluation, a decision's outcome...) and the source location. It expands
/// to nothing. `slint-sc-coverage` maps the LLVM coverage of the generated
/// code back to the `.slint` source by where these marks sit in it. A macro
/// other crates use has to be exported at the crate root; the generated code
/// refers to it through `private_unstable_api`.
#[macro_export]
#[doc(hidden)]
macro_rules! origin {
    ($origin:literal) => {};
