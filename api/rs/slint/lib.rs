...
pub mod wgpu {
    //! WGPU specific types and re-exports.
    //!
    //! *Note*: This module is behind a feature flag and may be removed or changed in future minor releases,
    //!         as new major WGPU releases become available.
    //!
    ...
    //! `Cargo.toml`:
    //! ```toml
    //! slint = { version = "~1.16", features = ["unstable-wgpu-29"] }
    //! ```
    //!
    //! `main.rs`:
    //!```rust,no_run
    ...
    //!fn main() -> Result<(), Box<dyn std::error::Error>> {
    //!    slint::BackendSelector::new()
    //!        .require_wgpu(slint::wgpu::WGPUConfiguration::default())
    //!        .select()?;
    //!    let app = HelloWorld::new()?;
    //!
    //!    let app_weak = app.as_weak();
    //!
    //!    app.window().set_rendering_notifier(move |state, graphics_api| {
    //!        let (Some(app), slint::RenderingState::RenderingSetup, slint::GraphicsAPI::WGPU{ device, queue, ..}) = (app_weak.upgrade(), state, graphics_api) else {
    //!            return;
    //!        };
    //!
    //!        let mut pixels = slint::SharedPixelBuffer::<slint::Rgba8Pixel>::new(320, 200);
    ...
    //!```
    //!
    pub use i_slint_core::graphics::wgpu::api::*;
}
