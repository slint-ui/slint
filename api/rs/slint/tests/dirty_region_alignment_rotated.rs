...
fn render_frame(
    ...
    switch_to_swapped_buffers: bool,
) {
...
fn alignment_uses_panel_axes_when_rotated() {
    ...
    render_frame(&window, &mut buffer, false);
    assert_frame(&buffer, 3);

    // The last window column is the last panel line, so the block the alignment adds there has
    // to stop at the edge of the buffer.
    for x in [5, 7, 9, 11, 15] {
        ui.set_rect_x(x as i32);
        render_frame(&window, &mut buffer, false);
        assert_frame(&buffer, x);
    }

    // Same, but with the previous frame's dirty region folded into this one.
    for x in [7, 15] {
        ui.set_rect_x(x as i32);
        render_frame(&window, &mut buffer, true);
        assert_frame(&buffer, x);
    }
}
