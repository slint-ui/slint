...
//! drag (like Qt/winit). The testing backend declines `start_drag`, so the `dragarea_*` cases
//! only cover the in-window fallback.
...
use slint::private_unstable_api::re_exports::{DragAction, DropEvent, MouseEvent};
...
slint::slint! {
    ...
    export component Target inherits Window {
        ...
        out property <bool> contains-drag <=> dropper.contains-drag;
        dropper := DropArea {
            width: 100%;
            height: 100%;
            can-drop(event) => {
...
fn native_drag_across_windows() {
    ...
    // hover, then Drop. The OS already vetted the allowed actions, so let any choice through.
    let mut drop = DropEvent::default();
    drop.data = data;
    drop.position = LogicalPosition::new(50.0, 50.0);
    drop.allow_copy = true;
    drop.allow_move = true;
    drop.allow_link = true;
    drop.proposed_action = DragAction::Copy;

    let target_inner = WindowInner::from_pub(target.window());
    target_inner.process_mouse_input(MouseEvent::DragMove(drop.clone()));
    assert!(target.get_contains_drag(), "the DropArea should report the hovering drag");

    let action = target_inner
        .process_mouse_input(MouseEvent::Drop(drop))
...
// in-window drag via `start_in_window_drag` — as the winit backend does when
// `event_loop.start_drag` errors. The in-window machinery then drives the drag to its drop.
#[test]
fn native_drag_falls_back_to_in_window() {
    ...
    // The native start "fails": fall back to the in-window drag, as the winit backend does.
    ...
    assert!(ui.get_contains_drag(), "the in-window drag should hover the DropArea");
