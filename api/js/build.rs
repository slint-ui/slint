...
fn main() {
    // The napi machinery and the generated TS language module only serve the
    // native (Node.js) build; wasm-pack builds must not write into the node
    // package.
...
fn generate_language_module() {
    ...
    ts.push_str("// AUTO-GENERATED by api/js/build.rs from internal/common/enums.rs\n");
    ...
    let out_dir =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("node").join("typescript").join("generated");
    ...
    match t {
        ...
        other => panic!(
            "Unmapped struct field type `{other}` in for_each_builtin_structs!. \
             Extend `map_field_type` in api/js/build.rs."
        ),
    }
}
    ...
    match t {
        ...
        other => panic!(
            "Unmapped struct field type `{other}` for default-value computation. \
             Extend `field_default` in api/js/build.rs."
        ),
    }
}
