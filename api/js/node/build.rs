...
fn generate_language_module() {
    ...
    ts.push_str("// AUTO-GENERATED by api/js/node/build.rs from internal/common/enums.rs\n");
    ...
    match t {
        ...
        other => panic!(
            "Unmapped struct field type `{other}` in for_each_builtin_structs!. \
             Extend `map_field_type` in api/js/node/build.rs."
        ),
    }
}
    ...
    match t {
        ...
        other => panic!(
            "Unmapped struct field type `{other}` for default-value computation. \
             Extend `field_default` in api/js/node/build.rs."
        ),
    }
}
