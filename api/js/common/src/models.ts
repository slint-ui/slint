...
import type { ModelBackend, ModelNotifyHandle } from "./backend";

let _modelBackend: ModelBackend | null = null;

/**
 * Sets the model notification backend. Must be called before creating any
 * Model instances.
 */
export function setModelBackend(backend: ModelBackend): void {
    _modelBackend = backend;
}

function getModelBackend(): ModelBackend {
    if (_modelBackend === null) {
        throw new Error(
            "Model backend not initialized. Call setModelBackend() first.",
        );
    }
    return _modelBackend;
}
...
export abstract class Model<T> implements Iterable<T> {
     ...
     * @hidden
     */
    modelNotify: ModelNotifyHandle;

    /**
     * @hidden
     */
    constructor(modelNotify?: ModelNotifyHandle) {
        this.modelNotify =
            modelNotify ?? getModelBackend().createModelNotify();
    }
    ...
    protected notifyRowDataChanged(row: number): void {
        getModelBackend().notifyRowDataChanged(this.modelNotify, row);
    }
    ...
    protected notifyRowAdded(row: number, count: number): void {
        getModelBackend().notifyRowAdded(this.modelNotify, row, count);
    }
    ...
    protected notifyRowRemoved(row: number, count: number): void {
        getModelBackend().notifyRowRemoved(this.modelNotify, row, count);
    }
    ...
    protected notifyReset(): void {
        getModelBackend().notifyReset(this.modelNotify);
    }
}
