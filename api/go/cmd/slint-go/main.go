...
func commonFlags(goos string) []string {
	switch goos {
	case "linux":
	...
	case "darwin":
		return []string{"-framework Cocoa", "-framework CoreGraphics", "-framework CoreText", "-framework Foundation", "-framework Carbon", "-framework CoreVideo", "-framework OpenGL"}
	default:
		return []string{}
	}
}

func sharedFlags(config *buildConfig, goos string) string {
	var flags []string

	if goos == "windows" {
		flags = []string{
			"${SRCDIR}/.slint-build/dist/" + config.targetID + "/slint_cpp.dll",
		}
	} else {
		flags = []string{
	...
	flags = append(flags, commonFlags(config.goos)...)

	return strings.Join(flags, " ")
}

func staticFlags(config *buildConfig, goos string, staticName string) string {
	flags := []string{
...
func runtimeFileContents(packageName string, config *buildConfig, slintStatic bool, staticName string, copied []string) string {
	...
	if slintStatic {
		flags = staticFlags(config, config.goos, staticName)
	} else {
		flags = sharedFlags(config, config.goos)
	}
