...
export class MapModel<T, U> extends NativeAdapterModel<U> {
     ...
     * @param sourceModel the wrapped model.
     * @param mapFunction maps the data from T to U. It must not modify `sourceModel`.
     */
    ...
    /**
     * Throws, because a MapModel can't map a `U` back to the `T` that its source model stores.
     */
    setRowData(_row: number, _data: U): void {
        throw new TypeError("setRowData is not supported on a MapModel");
    }
}
