...
define_cargo_dependent_feature(backend-winit-x11 "Enable support for the winit create to interact only with the X11 windowing system on Unix. Enable this option and turn off SLINT_FEATURE_BACKEND_WINIT for a smaller build with just X11 support on Unix." OFF "NOT SLINT_FEATURE_FREESTANDING AND NOT ANDROID AND NOT IOS")
define_cargo_dependent_feature(backend-winit-wayland "Enable support for the winit create to interact only with the wayland windowing system on Unix. Enable this option and turn off SLINT_FEATURE_BACKEND_WINIT for a smaller build with just wayland support." OFF "NOT SLINT_FEATURE_FREESTANDING AND NOT ANDROID AND NOT IOS")

define_cargo_dependent_feature(renderer-femtovg "Enable support for the OpenGL ES 2.0 based FemtoVG rendering engine." ON "NOT SLINT_FEATURE_FREESTANDING AND NOT ANDROID AND NOT IOS")
define_cargo_dependent_feature(renderer-femtovg-wgpu "Enable support for the WGPU based FemtoVG rendering engine." OFF "NOT SLINT_FEATURE_FREESTANDING AND NOT ANDROID AND NOT IOS")
if(ANDROID OR IOS)
...
define_cargo_dependent_feature(renderer-skia-opengl "Enable support for the Skia based rendering engine with its OpenGL backend." OFF "NOT SLINT_FEATURE_FREESTANDING AND NOT IOS")
define_cargo_dependent_feature(renderer-skia-vulkan "Enable support for the Skia based rendering engine with its Vulkan backend." OFF "NOT SLINT_FEATURE_FREESTANDING AND NOT IOS")
...
define_cargo_dependent_feature(renderer-vello "Enable support for the WGPU based vello rendering engine. Experimental: it is never selected automatically, set SLINT_BACKEND=winit-vello or SLINT_BACKEND=linuxkms-vello to use it." OFF "NOT SLINT_FEATURE_FREESTANDING AND NOT ANDROID AND NOT IOS")
...
define_cargo_dependent_feature(backend-qt "Enable Qt based rendering backend" OFF "NOT SLINT_FEATURE_FREESTANDING AND NOT ANDROID AND NOT IOS")

# Declared before the capabilities: it provides their default.
define_cargo_dependent_feature(backend-linuxkms "Enable support for the backend that renders a single window fullscreen on Linux. Turns SLINT_FEATURE_BACKEND_LINUXKMS_LIBSEAT and SLINT_FEATURE_BACKEND_LINUXKMS_LIBINPUT on by default; turn either off to build without that library" OFF "NOT SLINT_FEATURE_FREESTANDING AND NOT IOS")
...
define_cargo_dependent_feature(backend-linuxkms-libseat "Add libseat to the LinuxKMS backend, for GPU and input device access without root privileges. Enables the backend on its own and combines with SLINT_FEATURE_BACKEND_LINUXKMS_LIBINPUT" ${_slint_default_backend_linuxkms_capability} "NOT SLINT_FEATURE_FREESTANDING AND NOT IOS")
define_compat_option(BACKEND_LINUXKMS_NOSEAT BACKEND_LINUXKMS_LIBINPUT)
define_cargo_dependent_feature(backend-linuxkms-libinput "Add libinput to the LinuxKMS backend, to react to mouse, touch and keyboard input. Enables the backend on its own and combines with SLINT_FEATURE_BACKEND_LINUXKMS_LIBSEAT" ${_slint_default_backend_linuxkms_capability} "NOT SLINT_FEATURE_FREESTANDING AND NOT IOS")
