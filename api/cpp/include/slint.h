...
/// Access the layout cache of an item within a grid row repeater (stride-based).
/// Formula: cache[cache[jump_index] + repeater_index * stride + child_offset]
template<typename T>
inline T layout_cache_grid_row_access(const SharedVector<T> &cache, size_t jump_index,
                                      size_t repeater_index, size_t stride, size_t child_offset)
{
