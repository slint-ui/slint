...
const DOCS_PATH = process.env.SLINT_DOCS_PATH || '../docs/astro/src/content/docs';
...
class SlintDocsServer {
        ...
        tools: [
          {
            ...
            description: 'Search through Slint documentation for information about the Slint language, API reference, tutorials, and guides',
            inputSchema: {
              type: 'object',
              properties: {
                query: {
                ...
                category: {
                  type: 'string',
                  description: 'Optional category to filter results (guide, reference, tutorial)',
                  enum: ['guide', 'reference', 'tutorial', 'all'],
                  default: 'all',
                },
                limit: {
                  type: 'number',
          ...
          },
          {
            ...
            description: 'Get the full content of a specific Slint documentation page',
            inputSchema: {
              type: 'object',
              properties: {
                path: {
                  type: 'string',
                  description: 'The relative path to the documentation file (e.g., "guide/language/concepts/slint-language")',
                },
              },
              required: ['path'],
            },
          },
          {
            ...
            description: 'List all available documentation categories and their structure',
            inputSchema: {
              type: 'object',
              properties: {},
            },
      ...
      try {
        switch (name) {
          case 'search_slint_docs':
            return await this.searchDocs(args.query, args.category || 'all', args.limit || 10);
  ...
  loadDocs() {
    ...
    const loadDirectory = (dir, category = '') => {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory()) {
          loadDirectory(fullPath, category ? `${category}/${item}` : item);
        } else if (extname(item) === '.mdx' || extname(item) === '.md') {
          try {
            const content = readFileSync(fullPath, 'utf8');
            const { data: frontmatter, content: body } = matter(content);
            
            const relativePath = relative(resolvedDocsPath, fullPath);
            ...
            const doc = {
              ...
              description: frontmatter.description || this.extractDescription(textContent),
  ...
  extractDescription(content) {
    ...
    return sentences[0]?.trim().substring(0, 200) + '...' || 'No description available';
  }
...
${doc.content}`,
  ...
  async listCategories() {
    ...
    const formattedCategories = Object.entries(categories).map(([category, docs]) => {
      const docList = docs.map(doc => `  - [${doc.title}](${doc.url}) (\`${doc.path}\`)`).join('\n');
